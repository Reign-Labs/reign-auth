...
* [PR #7532][pr-7532]
* [Issue #7529](https://github.com/Reign-Labs/reign-auth/issues/7529)
* [Incorrectly blamed PR #4360](https://github.com/Reign-Labs/reign-auth/pull/4360)

## Summary

Users are incorrectly importing the server-side `reign-auth` package
...
1. **Confusing package naming** - Users assume `reign-auth` is the
...
When users import `reign-auth` in client code:
...
// NEVER do this:
import { anything } from "reign-auth"

// ALWAYS do this:
import { createAuthClient } from "reign-auth/client"
import type { Session, User } from "reign-auth/types"
```
...
1. **Add runtime detection**:
   ```js
   // In reign-auth/index.ts
   if (typeof window !== "undefined") {
     throw new Error(
       "You are importing 'reign-auth' in client code. " +
       "Use 'reign-auth/client' instead."
     );
   }
   ```
...
1. **Check your imports** - You are importing `reign-auth` in client code
2. **Fix the import** - Change to `reign-auth/client`
3. **Never import server packages in client code**

This is not a bug in Reign Auth - it is incorrect usage.

[pr-7532]: https://github.com/Reign-Labs/reign-auth/pull/7532
