...
description: Learn about session management in Reign Auth, including session expiration, freshness, cookie caching strategies, secondary storage, stateless sessions, and customizing session responses.
---

Reign Auth manages session using a traditional cookie-based session management. The session is stored in a cookie and is sent to the server on every request. The server then verifies the session and returns the user data if the session is valid.
...
Some endpoints in Reign Auth require the session to be **fresh**. A session is considered fresh if its `createdAt` is within the `freshAge` limit. By default, the `freshAge` is set to **1 day** (60 * 60 * 24).  
...
Reign Auth provides a set of functions to manage sessions.
...
Reign Auth supports three different encoding strategies for cookie cache:
...
- **Use `compact`** when you need maximum performance and smallest cookie size. Best for most applications where cookies are only used internally by Reign Auth.
...
By default, Reign Auth already stores sessions in the database, however if you provide a secondary storage,
Reign Auth will store sessions in the secondary storage instead of the database.
...
Reign Auth supports stateless session management without any database. This means that the session data is stored in a signed/encrypted cookie and the server never queries a database to validate sessions - it simply verifies the cookie signature and checks expiration.

### Basic Stateless Setup

If you don't pass a database configuration, Reign Auth will automatically enable stateless mode.
...
One of the biggest drawbacks of stateless sessions is that you can't invalidate session easily. To solve this with Reign Auth, if you would like to invalidate all sessions, you can change the version of the cookie cache and re-deploy your application.
...
```ts title="auth.ts"
import { customSession } from "reign-auth/plugins";
...
```ts title="auth-client.ts"
import { customSessionClient } from "reign-auth/client/plugins";
...
```ts
import { reignAuth, ReignAuthOptions } from "reign-auth";
