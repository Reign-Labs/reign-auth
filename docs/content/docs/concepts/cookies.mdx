---
title: Cookies
description: Learn how Reign Auth uses cookies, including cookie prefixes, custom cookie attributes, cross-subdomain sharing, secure cookies, and handling Safari ITP with proxies.
---

Cookies are used to store data such as session tokens, session data, OAuth state, and more. All cookies are signed using the `secret` key provided in the auth options or the `REIGN_AUTH_SECRET` environment variable. If you use [versioned secrets](/docs/reference/options#secrets) for rotation, encrypted cookie data (such as JWE session caches) will automatically use the current key and remain decryptable with previous keys.

### Cookie Prefix

By default, Reign Auth cookies follow the format `${prefix}.${cookie_name}`. The default prefix is "reign-auth". You can change the prefix by setting `cookiePrefix` in the `advanced` object of the auth options.
...
By default, Reign Auth uses the following cookies:
...
If your Reign Auth API is hosted on a different domain than your frontend, Safari may block authentication cookies entirely.
