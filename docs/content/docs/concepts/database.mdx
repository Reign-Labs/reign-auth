...
Reign Auth connects to a database to store data. The database will be used to store data such as users, sessions, and more. Plugins can also define their own database tables to store data.

You can pass a database connection to Reign Auth by passing a supported database instance in the database options. You can learn more about supported database adapters in the [Other relational databases](/docs/adapters/other-relational-databases) documentation.

<Callout type="info">
Reign Auth also works without any database. For more details, see [Stateless Session Management](/docs/concepts/session-management#stateless-session-management).
</Callout>

## CLI

Reign Auth comes with a CLI tool to manage database migrations and generate schema.
...
Reign Auth also provides a `generate` command to generate the schema required by Reign Auth. The `generate` command creates the schema required by Reign Auth. If you're using a database adapter like Prisma or Drizzle, this command will generate the right schema for your ORM. If you're using the built-in Kysely adapter, it will generate an SQL file you can run directly on your database.

```package-install
npx reign-auth@latest generate
```
...
<Callout>
  ...
  required by Reign Auth is described below and you can find additional schema
  required by plugins in the plugin documentation.
</Callout>

### Programmatic Migrations

In environments where the CLI isn't available (e.g. Cloudflare Workers, serverless functions), you can run migrations programmatically using `getMigrations` from `reign-auth/db/migration`.
...
Secondary storage in Reign Auth allows you to use key-value stores for managing session data, verification records, rate limiting counters, and other short-lived auth data. This can be useful when you want to offload the storage of intensive records to a high performance storage or even RAM.
...
Then, provide your implementation to the `reignAuth` function:
...
Reign Auth provides an official Redis storage package that uses [ioredis](https://github.com/redis/ioredis):

```bash
npm install @reign-auth/redis-storage ioredis
# or
pnpm add @reign-auth/redis-storage ioredis
```
...
export const auth = reignAuth({
	...
	secondaryStorage: redisStorage({
		client: redis,
		keyPrefix: "reign-auth:", // optional, defaults to "reign-auth:"
	}),
});
```
...
This implementation allows Reign Auth to use Redis for storing session data and rate limiting counters. You can also add prefixes to the keys names.
...
import { SecondaryStorage } from "reign-auth";
...
Finally, we can pass the implementation to the `reignAuth` function.
...
Reign Auth requires the following tables to be present in the database. The types are in `typescript` format. You can use corresponding types in your database.
...
Reign Auth allows you to customize the table names and column names for the core schema. You can also extend the core schema by adding additional fields to the user and session tables.
...
```ts title="auth.ts"
import { reignAuth } from "reign-auth";
import { twoFactor } from "reign-auth/plugins";
...
Reign Auth provides a type-safe way to extend the `user` and `session` schemas. You can add custom fields to your auth config, and the CLI will automatically update the database schema. These additional fields will be properly inferred in functions like `useSession`, `signUp.email`, and other endpoints that work with user or session objects.
...
Reign Auth by default will generate unique IDs for users, sessions, and other entities.
...
Database hooks allow you to define custom logic that can be executed during the lifecycle of core database operations in Reign Auth. You can create hooks for the following models: **user**, **session**, and **account**.
...
If you want to stop the database hook from proceeding, you can throw errors using the `APIError` class imported from `reign-auth/api`.
