---
title: API
description: Learn how to call Reign Auth API endpoints on the server, pass body, headers, and query parameters, retrieve response headers, and handle errors.
---

When you create a new Reign Auth instance, it provides you with an `api` object. This object exposes every endpoint that exists in your Reign Auth instance. And you can use this to interact with Reign Auth server side.

Any endpoint added to Reign Auth, whether from plugins or the core, will be accessible through the `api` object.
...
<Callout>
Reign Auth API endpoints are built on top of [reign-call](https://github.com/bekacru/reign-call), a tiny web framework that lets you call REST API endpoints as if they were regular functions and allows us to easily infer client types from the server.
</Callout>
...
```ts title="server.ts"
import { auth } from "@/lib/auth"
import { APIError, isAPIError } from "reign-auth/api";
