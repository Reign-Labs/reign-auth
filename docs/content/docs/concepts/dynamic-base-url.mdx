...
description: Configure Reign Auth to work with multiple domains and preview deployments.
---
...
Reign Auth supports dynamic base URL resolution using an **allowlist-based approach** that determines the correct URL from each incoming request.
...
When a request comes in, Reign Auth extracts the host from the `x-forwarded-host` or `host` header and validates it against your allowlist. If the host matches, that URL is used as the base URL for the request.
...
By default, if the incoming host doesn't match any pattern in `allowedHosts`, Reign Auth throws an error. You can provide a fallback URL instead:
...
<Callout type="info">
The `protocol` setting controls how the **URL is constructed** for each request. For the cookie `Secure` flag, Reign Auth uses `protocol: "https"` → secure, `protocol: "http"` → not secure, and for `"auto"` or unset → falls back to `NODE_ENV === "production"`. You can always override this with `advanced.useSecureCookies`.
</Callout>
...
When you configure `allowedHosts`, Reign Auth automatically adds them to `trustedOrigins`. This means:
...
If you need to share cookies across subdomains, you can enable `crossSubDomainCookies` without specifying a static `domain`. Reign Auth automatically derives the cookie domain from the resolved host of each request.
...
This differs from some other auth libraries that auto-trust platforms or accept any header value. Reign Auth prioritizes security over convenience.
