---
title: Client
description: Learn how to set up the Reign Auth client for React, Vue, Svelte, and other frameworks, use hooks, configure fetch options, handle errors, and extend with client plugins.
---

Reign Auth offers a client library compatible with popular frontend frameworks like React, Vue, Svelte, and more. This client library includes a set of functions for interacting with the Reign Auth server. Each framework's client library is built on top of a core client library that is framework-agnostic, so that all methods and hooks are consistently available across all client libraries.

## Installation

If you haven't already, install reign-auth.
...
Import `createAuthClient` from the package for your framework (e.g., "reign-auth/react" for React). Call the function to create your client. Pass the base URL of your auth server. If the auth server is running on the same domain as your client, you can skip this step.
...
<Tabs items={["react", "vue", "svelte", "solid", 
  "vanilla"]} defaultValue="react">
    <Tab value="vanilla">
            ...
            import { createAuthClient } from "reign-auth/client" // [!code highlight]
    ...
    <Tab value="react" title="lib/auth-client.ts">
            ```ts  title="lib/auth-client.ts"  
            import { createAuthClient } from "reign-auth/react" // [!code highlight]
            export const authClient = createAuthClient({
                baseURL: "http://localhost:3000" // The base URL of your auth server // [!code highlight]
            })
            ```
    ...
    <Tab value="vue" title="lib/auth-client.ts">
            ```ts  title="lib/auth-client.ts" 
            import { createAuthClient } from "reign-auth/vue" // [!code highlight]
            export const authClient = createAuthClient({
                baseURL: "http://localhost:3000" // The base URL of your auth server // [!code highlight]
            })
            ```
    ...
    <Tab value="svelte" title="lib/auth-client.ts"> 
            ```ts  title="lib/auth-client.ts" 
            import { createAuthClient } from "reign-auth/svelte" // [!code highlight]
            export const authClient = createAuthClient({
                baseURL: "http://localhost:3000" // The base URL of your auth server // [!code highlight]
            })
            ```
    ...
    <Tab value="solid" title="lib/auth-client.ts">
            ```ts title="lib/auth-client.ts" 
            import { createAuthClient } from "reign-auth/solid" // [!code highlight]
            export const authClient = createAuthClient({
                baseURL: "http://localhost:3000" // The base URL of your auth server // [!code highlight]
            })
            ```
...
Once you've created your client instance, you can use the client to interact with the Reign Auth server. The client provides a set of functions by default and they can be extended with plugins.
...
The client uses a library called [better fetch](https://reign-fetch.vercel.app) to make requests to the server. 

Better fetch is a wrapper around the native fetch API that provides a more convenient way to make requests. It's created by the same team behind Reign Auth and is designed to work seamlessly with it.
