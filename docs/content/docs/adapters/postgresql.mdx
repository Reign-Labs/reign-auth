---
title: PostgreSQL
description: Integrate Reign Auth with PostgreSQL.
---
...
Then, you can connect it straight into Reign Auth.
...
In most cases, the default schema is `public`. To have Reign Auth use a
...
The Reign Auth CLI migration system automatically detects your configured `search_path`:

- When running `npx reign-auth migrate`, it inspects only the tables in your configured schema
...
You can verify which schema Reign Auth will use by checking the `search_path`:
