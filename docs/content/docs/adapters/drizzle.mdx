...
description: Integrate Reign Auth with Drizzle ORM.
---
...
To use the Drizzle adapter, you need to install the `@reign-auth/drizzle-adapter` package:

```package-install
@reign-auth/drizzle-adapter
```
...
The [Reign Auth CLI](/docs/concepts/cli) allows you to generate or migrate
your database schema based on your Reign Auth configuration and plugins.

To generate the schema required by Reign Auth, run the following command:
...
<Callout type="warn">
  ...
  or run our latest CLI version `npx reign-auth@latest generate` to generate a new Drizzle schema with the relations.
...
export const user = mysqlTable("user", {
  ...
  // This allows drizzle & reign-auth to still use the original field name,
