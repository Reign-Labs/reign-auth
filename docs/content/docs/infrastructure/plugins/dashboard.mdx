---
title: Dashboard
description: The `dash()` plugin connects your Reign Auth instance to Reign Auth Infrastructure, enabling analytics tracking, activity monitoring, and admin dashboard APIs.
---

The Dashboard plugin is the core connection between your Reign Auth instance and Reign Auth Infrastructure. It powers the web dashboard with real-time data, tracks user activity, and enables admin APIs.
...
|--------|------|-------------|
| `apiUrl` | `string` | Reign Auth Infrastructure API URL |
...
import { dashClient } from "@reign-auth/infra/client";
...
Once `dash()` is active, the Reign Auth Infrastructure dashboard gives you:
