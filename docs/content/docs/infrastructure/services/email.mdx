---
title: Email Service
description: Reign Auth Infrastructure provides a managed transactional email service with pre-built templates for common authentication flows. Send verification emails, password resets, invitations, and more without managing email infrastructure.
---
...
The email service is included in the `@reign-auth/infra` package:

```ts
import { sendEmail, createEmailSender } from "@reign-auth/infra";
```
...
```ts
import { sendEmail } from "@reign-auth/infra";
...
```ts
import { createEmailSender } from "@reign-auth/infra";
...
The email service integrates seamlessly with Reign Auth's authentication flows. Here's a complete example:
