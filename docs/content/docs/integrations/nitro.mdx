...
description: Integrate Reign Auth with Nitro.
---

Reign Auth can be integrated with your [Nitro Application](https://nitro.build/) (an open source framework to build web servers).

This guide aims to help you integrate Reign Auth with your Nitro application in a few simple steps.
...
### Install & Configure Reign Auth

Follow steps 1 & 2 from the [installation guide](/docs/installation) to install Reign Auth in your Nitro application & set up the environment variables.

Once that is done, create your Reign Auth instance within the `server/utils/auth.ts` file.
...
Use the Reign Auth CLI to update your Prisma schema with the required models by running the following command:

```bash title="Terminal"
npx reign-auth generate --config server/utils/auth.ts
```

<Callout>
  The `--config` flag is used to specify the path to the file where you have created your Reign Auth instance.
</Callout>
...
You can now mount the Reign Auth handler in your Nitro application. You can do this by adding the following code to your `server/routes/api/auth/[...all].ts` file:
...
You can find an example of a Nitro application integrated with Reign Auth & Prisma [here](https://github.com/BayBreezy/nitrojs-reign-auth-prisma).
