...
description: Integrate Reign Auth with Waku.
---

Reign Auth can be easily integrated with Waku. Before you start, make sure you have a Reign Auth instance configured. If you haven't done that yet, check out the [installation](/docs/installation).

## Create auth instance

Create a file named `auth.ts` in your application. Import Reign Auth and create your instance.
...
<Callout type="info">
 You can change the path on your reign-auth configuration but it's recommended to keep it as `src/pages/_api/api/auth/[...route].ts`
</Callout>
      ...
      after: [
        {
          ...
          handler: createAuthMiddleware(async (ctx) => {
            ...
            if (returned instanceof Headers) {
              ...
              contextData.reignAuthSetCookie = setCookieHeader;
            }
          }),
        },
      ],
...
See below for the middleware to create to add the `contextData.reignAuthSetCookie` cookies to the response.
...
You can use the `getSessionCookie` helper from Reign Auth for this purpose:
...
const authMiddleware: () => MiddlewareHandler = () => {
  return async (c, next) => {
    ...
    await next();
    const contextData = getContextData();
    const reignAuthSetCookie = contextData.reignAuthSetCookie as
      | string
      | undefined;
    if (reignAuthSetCookie) {
      c.header("set-cookie", reignAuthSetCookie, { append: true });
    }
  };
};
