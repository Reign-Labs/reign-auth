...
description: Integrate Reign Auth with Electron.
---

Electron is a popular framework for building cross-platform desktop applications using web technologies.

Reign Auth can be integrated into Electron apps to provide secure authentication flows, leveraging the system browser.

## Installation

<Steps>
  <Step>
    ### Configure a Reign Auth front- & back-end
    
    Before integrating with Electron, ensure you have a Reign Auth server and client set up.
    
    To get started, check out our [installation](/docs/installation) guide for setting up Reign Auth.
  </Step>
  
  <Step>
    ### Install the required packages

    Install the Reign Auth server package and the Electron integration package in your server, Electron app, and web client projects.

    **In each project, run:**

    ```package-install
    reign-auth @reign-auth/electron
    ```
  ...
  <Step>
    ### Add the Electron plugin to your Reign Auth server
  ...
  <Step>
      ...
      export const authClient = createAuthClient({
        baseURL: "http://localhost:8081", // Base URL of your Reign Auth frontend
        plugins: [
          electronClient({
  ...
  <Step>
      ...
      To enable this, you need to add your app's protocol scheme to the `trustedOrigins` on your **Reign Auth server**.
      ...
      Then, update your Reign Auth config to include the scheme in `trustedOrigins`:
  ...
  <Step>
      ...
      <Callout type="warning">
        To run your process in `sandbox` mode, you need to ensure that `@reign-auth/electron` is bundled into the preload script.
        Import only `@reign-auth/electron/preload` inside the preload script with tree-shaking enabled, to avoid bundling unwanted dependencies.
          ...
          preload: {
            build: {
              externalizeDeps: { // [!code highlight]
                exclude: ["@reign-auth/electron"], // [!code highlight]
              }, // [!code highlight]
            },
          },
        });
        ...
        If your config uses a list of externals instead, do **not** add `@reign-auth/electron` to that list so it gets bundled.
      </Callout>
...
To handle already signed-in users, you can use the `electron.transferUser` method on the client and `electronTransferUser` on the server to transfer the user to the Electron app. [See Example](https://github.com/Reign-Labs/reign-auth/blob/main/demo/nextjs/app/(auth)/sign-in/_components/electron.tsx)
...
The prefix to use for cookies set by the plugin. (defaults to `reign-auth`)
...
Prefix for local storage keys. (defaults to `reign-auth`)
...
Prefix(es) for server cookie names to filter. (defaults to `reign-auth`)

This is used to identify which cookies belong to reign-auth to
...
```ts title="electron/auth-client.ts"
electronClient({
  cookiePrefix: ["reign-auth", "my-app"],
});
```

#### `channelPrefix?`

Channel prefix for IPC bridges. (defaults to `reign-auth`)
