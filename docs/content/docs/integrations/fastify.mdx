---
title: Reign Auth Fastify Integration Guide
description: Learn how to seamlessly integrate Reign Auth with your Fastify application.
---

This guide provides step-by-step instructions for configuring both essential handlers and CORS settings.

<Callout type="important">
A configured Reign Auth instance is required before proceeding. If you haven't set this up yet, please consult our [Installation Guide](/docs/installation).
</Callout>
...
Configure Reign Auth to process authentication requests by creating a catch-all route:

```ts title="server.ts"
import Fastify from "fastify";
import { auth } from "./auth"; // Your configured Reign Auth instance
...
To retrieve the user's session in your Fastify routes, use the `auth.api.getSession` method. Reign Auth provides a `fromNodeHeaders` helper function that converts Node.js request headers to the format expected by Reign Auth.
