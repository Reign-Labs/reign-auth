...
description: Integrate Reign Auth with Astro.
---

Reign Auth comes with first class support for Astro. This guide will show you how to integrate Reign Auth with Astro.
...
To enable Reign Auth to handle requests, we need to mount the handler to a catch all API route. Create a file inside `/pages/api/auth` called `[...all].ts` and add the following code:
...
declare namespace App {
    ...
    interface Locals {
        user: import("reign-auth").User | null;
        session: import("reign-auth").Session | null;
    }
}
```
