...
description: Integrate Reign Auth with Encore.
---

Reign Auth can be integrated with your [Encore](https://encore.dev) application (an open source TypeScript framework with automated infrastructure and observability).
...
To handle auth requests, mount Reign Auth on a catch-all endpoint using Encore's `api.raw()`:
...
<Callout>
Encore's `api.raw()` provides Node.js request/response types. We use `toNodeHandler` from `reign-auth/node` to bridge these to Reign Auth's Web API handler.
</Callout>
...
When requests come from a different origin, they are blocked by default. Add trusted origins to your Reign Auth config:
...
Encore has a built-in auth handler pattern for protecting endpoints. Create an auth handler that validates Reign Auth sessions:
...
For a complete walkthrough including database setup and deployment, see the [Reign Auth with Encore tutorial](https://encore.dev/blog/betterauth-tutorial).
