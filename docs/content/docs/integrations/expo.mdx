...
description: Integrate Reign Auth with Expo.
---

Expo is a popular framework for building cross-platform apps with React Native. Reign Auth supports both Expo native and web apps.
...
<Steps>
    <Step>
        ## Configure A Reign Auth Backend
        Before using Reign Auth with Expo, make sure you have a Reign Auth backend set up. You can either use a separate server or leverage Expo's new [API Routes](https://docs.expo.dev/router/reference/api-routes) feature to host your Reign Auth instance.

        To get started, check out our [installation](/docs/installation) guide for setting up Reign Auth on your server. If you prefer to check out the full example, you can find it [here](https://github.com/Reign-Labs/examples/tree/main/expo-example).

        To use the new API routes feature in Expo to host your Reign Auth instance you can create a new API route in your Expo app and mount the Reign Auth handler.

        ```ts title="app/api/auth/[...auth]+api.ts"
        import { auth } from "@/lib/auth"; // import Reign Auth handler
    ...
    <Step>
        ...
        Install both the Reign Auth package and Expo plugin into your server application.

        ```package-install
        reign-auth @reign-auth/expo
        ```
        <Callout type="info">
    ...
    <Step>
        ...
        - You also need to install both the Reign Auth package and Expo plugin into your Expo application.
    ...
    <Step>
        ...
        Add the Expo plugin to your Reign Auth server.
    ...
    <Step>
        ## Initialize Reign Auth Client
        
        To initialize Reign Auth in your Expo app, you need to call `createAuthClient` with the base URL of your Reign Auth backend. Make sure to import the client from `/react`.
        ...
        You need to also import client plugin from `@reign-auth/expo/client` and pass it to the `plugins` array when initializing the auth client.
        ...
        export const authClient = createAuthClient({
            baseURL: "http://localhost:8081", // Base URL of your Reign Auth backend.
            plugins: [
                expoClient({
                    scheme: "myapp",
                    storagePrefix: "myapp",
     ...
     <Step>
        ...
        Reign Auth uses deep links to redirect users back to your app after authentication. To enable this, you need to add your app's scheme to the `trustedOrigins` list in your Reign Auth config. 
        ...
        Then, update your Reign Auth config to include the scheme in the `trustedOrigins` list.
    ...
    <Step>
        ...
        Reign Auth relies on `package.json` exports to resolve its modules. Starting with **Expo SDK 53+** (including SDK 55), package exports support is enabled by default in Metro, so **no extra Metro configuration is needed**.
        ...
        // Do NOT set this to false — Reign Auth requires package exports
...
With Reign Auth initialized, you can now use the `authClient` to authenticate users in your Expo app.
...
This example shows how to implement Google Sign-In using an idToken. No additional configuration is needed in Reign Auth. However, since this approach uses native code, you'll need to configure Expo to obtain the idToken.

Follow the [Expo Google authentication guide](https://docs.expo.dev/guides/google-authentication/) to set up `@react-native-google-signin/google-signin`, then pass the idToken to Reign Auth:
...
Reign Auth provides a `useSession` hook to access the current user's session in your app.
...
**scheme**: scheme is used to deep link back to your app after a user has authenticated using oAuth providers. By default, Reign Auth tries to read the scheme from the `app.json` file. If you need to override this, you can pass the scheme option to the client.
...
**cookiePrefix**: Prefix(es) for server cookie names to identify which cookies belong to reign-auth. This prevents infinite refetching when third-party cookies are set. Can be a single string or an array of strings to match multiple prefixes. Defaults to `"reign-auth"`.
...
const authClient = createAuthClient({
    ...
    plugins: [
        expoClient({
            storage: SecureStore,
            // Multiple prefixes
            cookiePrefix: ["reign-auth", "my-app", "custom-auth"]
        })
    ]
});
```
