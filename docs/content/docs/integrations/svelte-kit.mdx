...
description: Integrate Reign Auth with SvelteKit.
---
...
export async function handle({ event, resolve }) {
  // Fetch current session from Reign Auth
...
You need to add it as a plugin to your Reign Auth instance.
...
import { sveltekitCookies } from "reign-auth/svelte-kit";
...
import { createAuthClient } from "reign-auth/svelte"; // make sure to import from reign-auth/svelte
