...
description: Integrate Reign Auth with Next.js.
---

Reign Auth can be easily integrated with Next.js. Before you start, make sure you have a Reign Auth instance configured. If you haven't done that yet, check out the [installation](/docs/installation).
...
<Callout type="info">
 You can change the path on your reign-auth configuration but it's recommended to keep it as `/api/auth/[...all]`
</Callout>
...
import { createAuthClient } from "reign-auth/react" // make sure to import from reign-auth/react
...
The client also uses [reign-fetch](https://github.com/bekacru/reign-fetch) to make the requests. You can pass the fetch configuration to the client.


## RSC and Server actions

The `api` object exported from the auth instance contains all the actions that you can perform on the server. Every endpoint made inside Reign Auth is a invocable as a function. Including plugins endpoints.
...
<Callout type="info">
**Migration from middleware:** Rename `middleware.ts` → `proxy.ts` and `middleware` → `proxy` function. All Reign Auth methods work identically.
</Callout>
...
<Callout>
  The example uses [reign-fetch](https://reign-fetch.vercel.app), but you can use any fetch library.
</Callout>
...
```ts title="middleware.ts"
import { getCookieCache } from "reign-auth/cookies";
...
Reign Auth is fully compatible with Next.js 16. The main change is that "middleware" is now called "proxy". See the [Auth Protection](#auth-protection) section above for Next.js 16+ proxy examples.
...
All Reign Auth methods work identically. See the [Next.js migration guide](https://nextjs.org/docs/app/api-reference/file-conventions/proxy#migration-to-proxy) for details.
