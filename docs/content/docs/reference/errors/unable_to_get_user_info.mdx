...
the authorization code for tokens, Reign Auth fetches the user's profile from the provider. If the
...
* Start the OAuth flow using Reign Auth methods so the correct scopes and parameters are used.
