...
`code` parameter (and typically `state`). Without the `code`, Reign Auth cannot exchange it for tokens,
...
* Start the flow through Reign Auth so the provider receives the correct parameters and the app expects a `code`.
