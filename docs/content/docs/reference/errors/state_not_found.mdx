...
During the OAuth callback, Reign Auth expects a `state` value to be present on the incoming request. 
...
### Start the flow via Reign Auth APIs

Always initiate OAuth through Reign Auth so we can generate and send `state` correctly. Avoid manually hitting 
callback endpoints or constructing authorize URLs unless you fully mirror Reign Auth's parameters.
