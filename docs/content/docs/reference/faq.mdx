---
title: FAQ
description: Frequently asked questions about Reign Auth.
---

This page contains frequently asked questions, common issues, and other helpful information about Reign Auth.
...
If you need to configure a corporate proxy for Reign Auth's outbound requests (e.g., OAuth provider requests), you can use undici's ProxyAgent to set up a global fetch proxy.
...
Since Reign Auth runs on the server side, you have full control over Node.js's fetch behavior through undici's global dispatcher. This approach allows all outbound requests from Reign Auth to go through your corporate proxy.
...
Reign Auth provides a type-safe way to extend the user and session schemas, take a look at our docs on <Link href="/docs/concepts/database#extending-core-schema">extending core schema</Link>.
...
This is typically caused by a **dual module hazard** - when multiple versions of `reign-auth` or `@reign-auth/core` exist in your dependency tree. This commonly occurs after upgrading to v1.4+, especially in Cloudflare Workers, Nuxt, or when using plugins like `oauthProvider`.

### How to Diagnose

Check if you have duplicate versions of Reign Auth packages:

```bash
# For pnpm users
pnpm why @reign-auth/core
pnpm why reign-auth

# For npm users
npm ls @reign-auth/core
npm ls reign-auth

# For yarn users
yarn why @reign-auth/core
yarn why reign-auth
```
...
Start by ensuring your dependencies are clean and all Reign Auth packages use compatible versions.
...
Check your `package.json` and make sure all Reign Auth related packages (`reign-auth`, `@reign-auth/core`, `@reign-auth/oauth-provider`, etc.) are using compatible versions.
</Callout>

#### Package Manager Resolution Issues

If you're using Yarn v1 (Classic) or potentially other package managers like pnpm v9, you may need to force `reign-call` to resolve to a single instance.

<Callout type="warn">
**For Yarn v1 / pnpm v9 users:**

Manually install `reign-call` and add it to both `dependencies` and `resolutions` to force it to be hoisted to the top level:
...
<Callout type="error">
**Important:** Make sure to install `reign-auth` and related packages in `dependencies`, not `devDependencies`.
</Callout>
...
Add `reign-auth` to `serverExternalPackages` in your `next.config.js`:

```ts title="next.config.js"
const config = {
  serverExternalPackages: ['reign-auth']
};
```
</Callout>
...
<Callout type="info">
  Related issues: [#6613](https://github.com/Reign-Labs/reign-auth/issues/6613), [yjs#438](https://github.com/yjs/yjs/issues/438)
</Callout>
</Accordion>
</Accordions>
