---
title: Options
description: Reign Auth configuration options reference.
---

List of all the available options for configuring Reign Auth. See [Reign Auth Options](https://github.com/Reign-Labs/reign-auth/blob/main/packages/core/src/types/init-options.ts).
...
Base URL for Reign Auth. This is typically the root URL where your application server is hosted. Note: If you include a path in the baseURL, it will take precedence over the default path.
...
If not explicitly set, the system will check for the environment variable `REIGN_AUTH_URL`. If that's also not set, it will be inferred from the incoming request.

<Callout type="warn">
Relying on request inference is not recommended. For security and stability, always set `baseURL` explicitly in your config or via the `REIGN_AUTH_URL` environment variable.
</Callout>

## `basePath`

Base path for Reign Auth. This is typically the path where the Reign Auth routes are mounted. It will be overridden if there is a path component within `baseURL`.
...
By default, Reign Auth will look for the following environment variables:
- `process.env.REIGN_AUTH_SECRET`
- `process.env.AUTH_SECRET`

If none of these environment variables are set, it will default to `"reign-auth-secret-12345678901234567890"`. In production, if it's not set, it will throw an error.
...
You can also set this via the `REIGN_AUTH_SECRETS` environment variable:

```txt title=".env"
REIGN_AUTH_SECRETS=2:new-secret-base64,1:old-secret-base64
```
...
Database configuration for Reign Auth.
...
Reign Auth supports various database configurations including [PostgreSQL](/docs/adapters/postgresql), [MySQL](/docs/adapters/mysql), and [SQLite](/docs/adapters/sqlite).
...
```ts
import { reignAuth } from "reign-auth";
import { emailOTP } from "reign-auth/plugins";
...
When this option is enabled, you can read the decrypted account cookie in hooks or middleware using `getAccountCookie` from `reign-auth/cookies`:

```ts
import { createAuthMiddleware } from "reign-auth/api";
import { getAccountCookie } from "reign-auth/cookies";
...
If `secondaryStorage` is configured, verification records are stored there by default. That behavior applies to flows that use Reign Auth's shared verification layer, including OTP and magic-link style flows.
...
Logger configuration for Reign Auth.
...
The logger configuration allows you to customize how Reign Auth handles logging. It supports the following options:
...
- `customizeDefaultErrorPage`: Configure the default error page provided by Reign Auth. Start your dev server and go to `/api/auth/error` to see the error page.
...
Enable or disable Reign Auth's telemetry collection. (default: `false`)
