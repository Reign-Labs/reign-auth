---
title: Migrating from WorkOS to Reign Auth
description: A step-by-step guide to transitioning from WorkOS to Reign Auth.
---

In this guide, we’ll walk through how to migrate a project from WorkOS to Reign Auth, covering how to move a basic WorkOS setup integrated with a Next.js app and the key considerations to keep in mind.

## Before we begin

Before getting started, let’s review which WorkOS authentication features are fully or partially supported in Reign Auth. If a feature you use in WorkOS is available via a plugin, you’ll need to configure it in the next step.

<Accordions type="multiple">
<Accordion title="🟢 Supported">
<>
| from WorkOS | to Reign Auth |
|---------|-------------|
...
First, set up Reign Auth in your project. Follow the [installation guide](/docs/installation) to get started.

### Database

Reign Auth supports various databases. Set up your preferred database. In this guide, we’ll use PostgreSQL with the default database adapter.
...
Set up the social providers you used in WorkOS as follows. Reign Auth supports a wider range of providers, so you can add more if needed. Since WorkOS ensures emails are unique, configure `account.accountLinking` in Reign Auth to ensure the same behavior.
...
You probably used metadata in WorkOS. To preserve that metadata and the user id from WorkOS (e.g., user_01KBT4BMFF7ASGRDD0WZ6W63FF), extend the `user` schema as shown below. Reign Auth provides a more flexible way to store user data. For more information, see [here](/docs/concepts/database#extending-core-schema).
...
Refer to the [section](#before-we-begin) mapping WorkOS features to Reign Auth. If a feature you used in WorkOS is available as a Reign Auth plugin, add it to the plugin options. Reign Auth provides a wider range of out-of-the-box features through plugins. For more information, see [here](/docs/concepts/plugins).

```ts title="auth.ts"
import { reignAuth } from "reign-auth";
import { haveIBeenPwned } from "reign-auth/plugins/haveibeenpwned";
import { Pool } from "pg";
...
Reign Auth allows you to control your own database, and you can easily generate the appropriate schema for your auth instance using the CLI. For more information, see [here](/docs/concepts/cli).

### Default database adapter

Run the `migrate` command to create the schema for your Reign Auth instance in the database.

```package-install
npx reign-auth migrate
```
...
- This migration script covers the common cases of managing users with email+password and social login. For features like SSO or CLI Auth, which are provided as plugins in Reign Auth, be sure to update the script based on the examples.
</Callout>
...
In WorkOS, the auth API was provided as a managed service. With Reign Auth, the auth API now lives directly within your application.
...
In Reign Auth, instead of fetching these values via an API, you can create the pages at your desired paths and use them directly.
</Step>
...
WorkOS provides Proxy (Middleware) authentication. Reign Auth doesn’t recommend protecting resources directly in middleware, so we don't provide dedicated helpers for that.
...
In Reign Auth, for convenience rather than resource protection, the proxy (middleware) can be used as follows. This is supported in Next.js 15+ with the Node.js runtime.
...
In WorkOS, if resources were protected on each page, you can update the logic in Reign Auth as follows.

#### Server-side

<Tabs items={["WorkOS", "Reign Auth"]}>
<Tab value="WorkOS">
...
<Callout type="info">
If options like `ensureSignedIn` were convenient in WorkOS, you can create a reusable helper like `ensureSession()` in Reign Auth.
</Callout>
...
WorkOS is a managed service and keeps your data in sync with your server through APIs or Webhooks. With Reign Auth, you fully own your authentication system and can manage data freely through the API. However, if you previously relied on Webhooks for synchronization, additional adjustments will be needed.
...
Congratulations! You've successfully migrated from WorkOS to Reign Auth. Reign Auth offers greater flexibility and more features, so be sure to explore the [documentation](/docs) to unlock its full potential.
