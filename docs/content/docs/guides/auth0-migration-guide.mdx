---
title: Migrating from Auth0 to Reign Auth
description: A step-by-step guide to transitioning from Auth0 to Reign Auth.
---

In this guide, we'll walk through the steps to migrate a project from Auth0 to Reign Auth — including email/password with proper hashing, social/external accounts, two-factor authentication, and more.
...
import { Pool } from "pg";
import { reignAuth } from "reign-auth";
import { admin, twoFactor, username } from "reign-auth/plugins";
...
import { generateRandomString, symmetricEncrypt } from "reign-auth/crypto";
...
function mapAuth0RoleToReignAuthRole(auth0Roles: string[]) {
    if (typeof auth0Roles === 'string') return auth0Roles;
    if (Array.isArray(auth0Roles)) return auth0Roles.join(',');
}
// helper function to migrate password from auth0 to Reign Auth for custom hashes and algs
            ...
            try {
                ...
                for (const member of members.data || []) {
                    try {
                        ...
                        const role = mapAuth0RoleToReignAuthRole(userRoles.data?.map(r => r.name) || []);
                        await ctx.adapter.create({
                            model: "member",
                            data: {
            ...
            try {
                ...
                const baseUserData = {
                    ...
                    ...(isAdminEnabled ? {
                        ...
                        role: mapAuth0RoleToReignAuthRole(auth0User.roles || []),
                    } : {}),
...
3. Verify the migrated data in Reign Auth before proceeding
...
<Step>
 ...
 By default, Reign Auth uses the `scrypt` algorithm to hash passwords. Since Auth0 uses `bcrypt`, you'll need to configure Reign Auth to use bcrypt for password verification.
...
Now that the data is migrated, update your components to use Reign Auth. Here's an example for the sign-in component:

```tsx title="components/auth/sign-in.tsx"
import { authClient } from "reign-auth/client";
...
Replace your Auth0 middleware with Reign Auth's middleware:
...
Once you've verified everything is working correctly with Reign Auth, remove Auth0:
...
### Role Mapping
The script includes a basic role mapping function (`mapAuth0RoleToReignAuthRole`). Customize this function based on your Auth0 roles and Reign Auth role requirements.
...
Now! You've successfully migrated from Auth0 to Reign Auth.
