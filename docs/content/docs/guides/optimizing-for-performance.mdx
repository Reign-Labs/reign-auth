...
description: A guide to optimizing your Reign Auth application for performance.
---

In this guide, we’ll go over some of the ways you can optimize your application for a more performant Reign Auth app.

## Caching

Caching is a powerful technique that can significantly improve the performance of your Reign Auth application by reducing the number of database queries and speeding up response times.
...
Optimizing database performance is essential to get the best out of Reign Auth.
...
If you're using custom adapters (like Prisma, Drizzle, or MongoDB), you can reduce your bundle size by using `reign-auth/minimal` instead of `reign-auth`. This version excludes Kysely, which is only needed when using direct database connections.

### Usage

Simply import from `reign-auth/minimal` instead of `reign-auth`:

<Tabs items={["Prisma", "Drizzle", "MongoDB"]}>
  <Tab value="Prisma">
    ```ts title="auth.ts"
    import { reignAuth } from "reign-auth/minimal"; // [!code highlight]
...
<Callout type="warning">
  **Limitations:**
  ...
  - Built-in migrations are not supported. Use external migration tools (or use `reign-auth` if you need built-in migration support)
</Callout>
