...
description: A step-by-step guide to creating your first Reign Auth plugin.
---

In this guide, we’ll walk you through the steps of creating your first Reign Auth plugin.


<Callout type="warn">
This guide assumes you have <Link href="/docs/installation">setup the basics</Link> of Reign Auth and are ready to create your first plugin.
</Callout>
...
<Step>
## Server plugin first
Reign Auth plugins operate as a pair: a <Link href="/docs/concepts/plugins#create-a-server-plugin">server plugin</Link> and a <Link href="/docs/concepts/plugins#creating-a-client-plugin">client plugin</Link>.
...
By creating a schema here, this also allows <Link href="/docs/concepts/cli">Reign Auth’s CLI</Link> to generate the schemas required to update your database.
...
```ts title="Imports"
import { createAuthMiddleware, APIError } from "reign-auth/api";
```
```ts title="Before hook"
{
  //...
...
Congratulations! You’ve successfully created your first ever Reign Auth plugin.
...
our <Link href="#">Discord server</Link>,
or through a <Link href="https://github.com/Reign-Labs/reign-auth/pulls">pull-request</Link>
