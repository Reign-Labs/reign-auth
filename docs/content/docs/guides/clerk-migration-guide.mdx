---
title: Migrating from Clerk to Reign Auth
description: A step-by-step guide to transitioning from Clerk to Reign Auth.
---

In this guide, we'll walk through the steps to migrate a project from Clerk to Reign Auth — including email/password with proper hashing, social/external accounts, phone number, two-factor data, and more.
...
Before starting the migration process, set up Reign Auth in your project. Follow the [installation guide](/docs/installation) to get started. And go to 
...
<Callout type="info">
**Important for Clerk Migrations**: Clerk uses bcrypt to hash passwords, while Reign Auth uses `scrypt` by default. To ensure migrated users can sign in with their existing passwords, you'll need to configure Reign Auth to use bcrypt for password verification.
</Callout>
...
Then configure Reign Auth to use bcrypt:
...
import { Pool } from "pg";
import { reignAuth } from "reign-auth";
import { admin, twoFactor, phoneNumber, username } from "reign-auth/plugins";
import bcrypt from "bcrypt";
...
<Step>
  ...
  Now that the data is migrated, you can start updating your components to use Reign Auth. Here's an example for the sign-in component:
...
<Step>
  ...
  Replace your Clerk middleware with Reign Auth's middleware:
  ...
  Once you've verified that everything is working correctly with Reign Auth, you can remove Clerk:
...
[Goodbye Clerk, Hello Reign Auth – Full Migration Guide!](https://www.youtube.com/watch?v=Za_QihbDSuk)

## Wrapping Up

Congratulations! You've successfully migrated from Clerk to Reign Auth.
