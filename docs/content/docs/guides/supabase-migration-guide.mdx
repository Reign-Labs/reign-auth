---
title: Migrating from Supabase Auth to Reign Auth
description: A step-by-step guide to transitioning from Supabase Auth to Reign Auth.
---

In this guide, we'll walk through the steps to migrate a project from Supabase Auth to Reign Auth. 
...
import { reignAuth } from 'reign-auth';
import { Pool } from "pg";
import { admin, anonymous, phoneNumber } from 'reign-auth/plugins';
...
This will create the necessary tables in the `public` schema of your Reign Auth instance.

Now that we have the necessary tables in our database, we can run the migration script to migrate the users and accounts from Supabase to Reign Auth.
...
import { generateId } from 'reign-auth';
import { DBFieldAttribute } from 'reign-auth/db';
import { Pool } from 'pg';
import { auth } from './auth'; // <- Your Reign Auth Instance
...
const CONFIG = {
   ...
   * Phone-only users need an email for Reign Auth
   * Format: {phone_number}@{tempEmailDomain}
   */
  tempEmailDomain: 'temp.auth.reign-labs.com',
};
...
/**
 * Validates that the imported Reign Auth instance meets migration requirements
 */
async function validateAuthConfig() {
  ...
  if (errors.length > 0) {
    console.error('\n🟧 Reign Auth Configuration Errors:\n');
    errors.forEach((err) => console.error(`   ${err}`));
    console.error('\n🟧 Please update your Reign Auth configuration to include:\n');
...
async function migrateFromSupabase() {
  ...
  // Validate Reign Auth configuration
...
async function main() {
  console.log('🚀 Supabase Auth → Reign Auth Migration\n');
...
- `tempEmailDomain`: Temporary email domain for phone-only users. Default: "temp.auth.reign-labs.com"
</Step>
...
Run the migration script to migrate the users and accounts from Supabase to Reign Auth.
...
<Step>
 ...
 By default, Reign Auth uses the `scrypt` algorithm to hash passwords. Since Supabase uses `bcrypt`, you'll need to configure Reign Auth to use bcrypt for password verification.
...
Update your codebase from Supabase auth calls to Reign Auth API.

Here's a list of the Supabase auth API calls and their Reign Auth counterparts.
...
Install the Reign Auth SSO package:

```package-install
npm install @reign-auth/sso
```
...
/**
 * Migrates Supabase SSO providers to Reign Auth format
 * 
 * @param providers - Array of exported Supabase SSO providers
 * @param reignAuthUrl - Your Reign Auth base URL (e.g., https://app.example.com)
 */
async function migrateSSOProviders(
  ...
  for (const provider of providers) {
    ...
    // Transform Supabase attribute mapping to Reign Auth format
    ...
    const samlConfig = {
      ...
      callbackUrl: `${reignAuthUrl}/api/auth/sso/saml2/callback/${providerId}`,
      // IdP metadata - Reign Auth will fetch and parse this
      ...
      spMetadata: {
        entityID: `${reignAuthUrl}/api/auth/sso/saml2/sp/metadata`,
      },
      // Name ID format from Supabase config
      identifierFormat: provider.saml.name_id_format || 
        'urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress',
      // Map Supabase attribute names to Reign Auth fields
      mapping: {
        id: 'nameID',
    ...
    const ssoProviderData: SSOProviderInsertData = {
      ...
      organizationId: null, // Set this if linking to a Reign Auth organization
...
// Your Reign Auth application URL
const REIGN_AUTH_URL = process.env.REIGN_AUTH_URL || 'https://app.example.com';
...
async function main() {
  ...
  await migrateSSOProviders(providers, REIGN_AUTH_URL);
}
...
After migrating, you need to update your IdP (Okta, Azure AD, Google Workspace, etc.) with the new Reign Auth endpoints.

**Update these settings in your IdP:**

| Setting | Old Value (Supabase) | New Value (Reign Auth) |
|---------|----------------------|-------------------------|
...
Replace Supabase SSO authentication calls with Reign Auth:
...
- [ ] Providers migrated to Reign Auth database
- [ ] User accounts linked to SSO providers
- [ ] IdP settings updated (ACS URL, Entity ID)
- [ ] Client code updated to use Reign Auth SSO
...
Congratulations! You've successfully migrated from Supabase Auth to Reign Auth.

Reign Auth offers greater flexibility and more features—be sure to explore the [documentation](/docs) to unlock its full potential.
