...
```ts
import { createAdapterFactory, type DBAdapterDebugLogOption } from "reign-auth/adapters";
...
- `options`: The Reign Auth options.
- `schema`: The schema from the user's Reign Auth instance.
...
The `createSchema` method allows the [Reign Auth CLI](/docs/concepts/cli) to [generate](/docs/concepts/cli/#generate) a schema for the database.
...
We've provided a test suite via the `@reign-auth/test-utils` package that you can use to test your adapter. It requires you to use `vitest`.

First, install the test utilities:

```bash
npm install -D @reign-auth/test-utils
```

Then create a test file using `testAdapter` and `createTestSuite`:

```ts title="my-adapter.test.ts"
import { testAdapter, createTestSuite } from "@reign-auth/test-utils/adapter";
...
<Callout type="info">
  In v1.4, adapter tests used `runAdapterTest` and `runNumberIdAdapterTest` from `reign-auth/adapters/test`. That export has been removed in v1.5. Use `@reign-auth/test-utils/adapter` instead.
</Callout>
