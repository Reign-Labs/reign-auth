---
title: MCP
description: MCP provider plugin for Reign Auth
---
...
<Steps>
    <Step>
        ...
        import { mcp } from "reign-auth/plugins"; // [!code highlight]
...
import { oAuthProtectedResourceMetadata } from "reign-auth/plugins";
import { auth } from "@/lib/auth";
...
import { withMcpAuth } from "reign-auth/plugins";
import { z } from "zod";
...
The examples above use `withMcpAuth` which requires the Reign Auth instance to be in the **same process** as the MCP server. If your MCP server runs as a separate service (different repo, different runtime, different language), you can use the **MCP Client** — a lightweight, framework-agnostic HTTP client that validates Bearer tokens against a remote Reign Auth server.

<Callout>
    No additional packages needed — the MCP Client is included in `reign-auth`.
</Callout>
...
<Steps>
    <Step>
        ...
        Point it at your Reign Auth server's URL (the same `baseURL` + `basePath` from your auth config):

        ```ts title="mcp-server.ts"
        import { createMcpAuthClient } from "reign-auth/plugins/mcp/client" // [!code highlight]
    ...
    <Step>
        ...
        // GET /.well-known/oauth-authorization-server → proxied from Reign Auth
        // GET /.well-known/oauth-protected-resource → points MCP clients to Reign Auth
        ```

        These proxy and cache the metadata from your Reign Auth server, so MCP clients can discover the authorization, token, and registration endpoints automatically.
    </Step>
</Steps>
...
<Tabs items={["Hono", "Express", "Official MCP SDK", "mcp-use"]}>
    <Tab value="Hono">
        ...
        import { mcpAuthHono } from "reign-auth/plugins/mcp/client/adapters" // [!code highlight]
    ...
    <Tab value="Official MCP SDK">
        ...
        import { mcpAuthOfficial } from "reign-auth/plugins/mcp/client/adapters" // [!code highlight]
    ...
    <Tab value="mcp-use">
        ...
        import { mcpAuthMcpUse } from "reign-auth/plugins/mcp/client/adapters" // [!code highlight]
...
<TypeTable
  type={{
    authURL: {
        description: "Full URL to Reign Auth endpoints (baseURL + basePath)",
        type: "string",
        required: true
    },
    resource: {
