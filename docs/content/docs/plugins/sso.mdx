...
<Steps>
    ...
    <Step>
        ...
        import { sso } from "@reign-auth/sso"; // [!code highlight]
    ...
    <Step>
        ...
        import { ssoClient } from "@reign-auth/sso/client" // [!code highlight]
...
<Callout type="info">
When you register an OIDC provider, Reign Auth automatically fetches and validates the IdP's [OIDC discovery document](#oidc-discovery). Most endpoint fields are optional — see [OIDC Discovery](#oidc-discovery) for details on auto-discovered fields and possible registration errors.
</Callout>
...
Reign Auth automatically fetches and validates the provider's [OpenID Connect Discovery Document](https://openid.net/specs/openid-connect-discovery-1_0.html) from:
...
Reign Auth fills in the following fields by reading the IdP's discovery document (if not explicitly provided):
...
Reign Auth validates that the IdP's metadata is correct and complete **before** allowing registration. This prevents subtle runtime failures during sign-in or token validation.

<Callout type="info">
Reign Auth does **not** support implicit-only OIDC flows.
...
| `unsupported_token_auth_method` | The IdP only supports token auth methods that Reign Auth doesn't support |
...
- Reign Auth automatically performs OIDC discovery at registration time
...
If you're using [Reign Auth Infrastructure](/docs/plugins/sso), you get access to a self-service SSO dashboard that simplifies onboarding enterprise customers. Instead of manually exchanging SAML metadata and certificates, organization admins can generate a shareable onboarding link that guides enterprise IT teams through configuring their identity provider.

The dashboard is available at:

```
your-dashboard-url/organization/[orgId]/enterprise
```
...
Reign Auth provides flexibility to support both:
...
Reign Auth validates SAML cryptographic algorithms and warns about deprecated ones (SHA-1, RSA 1.5, 3DES) by default.
...
Reign Auth enforces size limits on SAML payloads to protect against denial-of-service attacks via oversized XML.
...
<Callout type="info">
When using a full URL, make sure it routes to your Reign Auth instance.
</Callout>
...
To do this, you'll need to add a `TXT` record to your domain's DNS settings:
    ...
    *   **Host:** `_reign-auth-token-{your-provider-id}` (**Note:** An underscore is automatically prepended to follow DNS infrastructure subdomain conventions. The `reign-auth-token` part can be customized through the `domainVerification.tokenPrefix` option)
...
**Important**: The `callbackUrl` in your SAML configuration should point to your application's destination URL (e.g., `/dashboard`), **not** the callback route itself. Reign Auth automatically handles the callback route and redirects users to your specified `callbackUrl` after successful authentication.
...
Reign Auth supports **IdP-initiated SSO flows**, where users access your application directly from their Identity Provider dashboard (e.g., Okta, Azure AD, OneLogin). This is common in enterprise environments where IT admins prefer centralized app access.

**How it works:**

1. User clicks your app icon in the IdP dashboard
2. IdP POSTs SAMLResponse to `/api/auth/sso/saml2/callback/{providerId}`
3. Reign Auth processes the assertion, creates a session, and redirects to your `callbackUrl`
...
<Callout type="info">
If you previously created a manual GET handler for the SAML callback route as a workaround, you can remove it after upgrading. Reign Auth now handles GET requests automatically.
</Callout>

<Callout type="info">
**Security:** Reign Auth validates all redirect URLs to prevent open redirect attacks. Only relative paths (e.g., `/dashboard`) and URLs matching your configured `trustedOrigins` are allowed. Malicious URLs like `https://evil.com` or protocol-relative URLs (`//evil.com`) are automatically blocked.
</Callout>
