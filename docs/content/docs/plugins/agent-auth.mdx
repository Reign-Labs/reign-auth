...
<Callout type="warn">
This plugin is an implementation of a standard on heavy development. It's not yet stable and may change in the future. Please report any issues or bugs on [Github](https://github.com/Reign-Labs/agent-auth).
</Callout>

The Agent Auth plugin lets your Reign Auth server act as an **Agent Auth provider**. It's a server implementation of the [Agent Auth Protocol](https://agentauthprotocol.com).
...
<Steps>
    ...
    <Step>
        ...
        import { agentAuth } from "@reign-auth/agent-auth"; // [!code highlight]
    ...
    <Step>
        ### Optional: add the Reign Auth client plugin

        If you want type-safe access to the plugin endpoints from a Reign Auth client, add the client plugin too.

        ```ts title="auth-client.ts"
        import { createAuthClient } from "reign-auth/client";
        import { agentAuthClient } from "@reign-auth/agent-auth/client"; // [!code highlight]
...
- **`issuer`** — The provider’s base URL (Reign Auth `baseURL`).
...
<Callout>
  The discovery route should live at `/.well-known/agent-configuration`, even if your Reign Auth base path is `/api/auth`.
</Callout>
...
```ts title="auth.ts"
import { reignAuth } from "reign-auth";
import { agentAuth } from "@reign-auth/agent-auth";
import { createFromOpenAPI } from "@reign-auth/agent-auth/openapi";
...
} from "@reign-auth/agent-auth/openapi";
...
import { verifyAgentRequest } from "@reign-auth/agent-auth";
...
Types are exported from **`@reign-auth/agent-auth`** (for example **`AgentSession`**).
