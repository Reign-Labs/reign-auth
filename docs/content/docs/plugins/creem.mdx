...
- **Flexible Architecture** - Use Reign Auth endpoints or direct server-side functions
...
Configure Reign Auth with the Creem plugin:

```typescript
// lib/auth.ts
import { reignAuth } from "reign-auth";
import { creem } from "@creem_io/reign-auth";
...
import { creemClient } from "@creem_io/reign-auth/client";
...
```typescript
// lib/auth-client.ts
import { createCreemAuthClient } from "@creem_io/reign-auth/create-creem-auth-client";
import { creemClient } from "@creem_io/reign-auth/client";
...
<Callout type="info">
  Depending on your database adapter, additional setup steps may be required. Refer to the [Reign Auth adapter documentation](https://www.auth.reign-labs.com/docs/adapters/mysql) for details.
</Callout>
...
<Steps>
  <Step>
    ...
    (`/api/auth` is the default Reign Auth server path)
...
import { validateWebhookSignature } from "@creem_io/reign-auth/server";
...
Use these utilities directly in Server Components, Server Actions, or API routes without going through Reign Auth endpoints.
...
import {
  ...
  getDaysUntilRenewal,
  validateWebhookSignature,
} from "@creem_io/reign-auth/server";
```

### Server Component Example

```typescript
import { checkSubscriptionAccess } from "@creem_io/reign-auth/server";
...
import { createCheckout } from "@creem_io/reign-auth/server";
import { auth } from "@/lib/auth";
import { headers } from "next/headers";
import { redirect } from "next/navigation";
...
2. Run migrations: `npx reign-auth migrate`
...
- [Reign Auth Documentation](https://auth.reign-labs.com)
