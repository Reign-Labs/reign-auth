...
<Steps>
    <Step>
        ...
        ```package-install
        npm install @reign-auth/passkey
        ```
    </Step>
    <Step>
        ...
        import { passkey } from "@reign-auth/passkey" // [!code highlight]
    ...
    <Step>
        ...
        import { passkeyClient } from "@reign-auth/passkey/client" // [!code highlight]
...
**origin**: The origin URL at which your reign-auth server is hosted. `http://localhost` and `http://localhost:PORT` are also valid. Do NOT include any trailing /.
...
- `webAuthnChallengeCookie`: Cookie name for storing WebAuthn challenge ID during authentication flow (Default: `reign-auth-passkey`)
...
By default, the passkey plugin uses `"reign-auth-passkey"` as the challenge cookie name. Since this starts with `"reign-auth"`, it will work with the default Expo client configuration. However, if you customize the `webAuthnChallengeCookie` option, you must also update the `cookiePrefix` in your Expo client configuration.
