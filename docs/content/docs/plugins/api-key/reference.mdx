...
<Callout>
  When `storage` is set to `"secondary-storage"`, you must configure `secondaryStorage` in your Reign Auth options. API keys will be stored using key-value patterns:
