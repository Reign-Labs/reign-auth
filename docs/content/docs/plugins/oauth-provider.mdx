...
description: A Reign Auth plugin that enables your auth server to serve as an OAuth 2.1 provider.
---
...
<Steps>
  <Step>
    ...
    import { jwt } from "reign-auth/plugins";
    import { oauthProvider } from "@reign-auth/oauth-provider"; // [!code highlight]
...
import { oauthProviderClient } from "@reign-auth/oauth-provider/client" // [!code highlight]
...
import { createAuthClient } from "reign-auth/client";
import { oauthProviderResourceClient } from "@reign-auth/oauth-provider/resource-client" // [!code highlight]
...
To enable client registration set `allowDynamicClientRegistration: true` in your ReignAuth config.
...
import { oauthProviderOpenIdConfigMetadata } from "@reign-auth/oauth-provider";
import { auth } from "@/lib/auth";
...
import { oauthProviderAuthServerMetadata } from "@reign-auth/oauth-provider";
import { auth } from "@/lib/auth";
...
Verification can be performed using `verifyAccessToken` available through the `oauthProviderResourceClient` plugin or `reign-auth/oauth2` package.

With `reign-auth` package:
...
<Callout type="warn">
These rate limits only apply when Reign Auth's global rate limiting is enabled. By default, rate limiting is only enabled in production. See [Rate Limiting](/docs/concepts/rate-limit) for global configuration.
</Callout>
...
<Callout type="info">
Setting an endpoint to `false` removes the OAuth Provider's stricter per-endpoint limit. The endpoint will still be subject to Reign Auth's global rate limiting if enabled.
</Callout>
