---
title: Email OTP
description: Email OTP plugin for Reign Auth.
---
...
<Steps>
  <Step>
    ...
    import { emailOTP } from "reign-auth/plugins" // [!code highlight]
    ...
    import { emailOTPClient } from "reign-auth/client/plugins" // [!code highlight]
...
- `storeOTP`: The method used to transform the OTP before it is stored by Reign Auth's verification layer, whether `encrypted`, `hashed` or `plain` text. Default is `plain` text.
