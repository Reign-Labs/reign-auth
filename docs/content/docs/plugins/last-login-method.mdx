...
<Steps>
    <Step>
        ...
        import { lastLoginMethod } from "reign-auth/plugins" // [!code highlight]
    ...
    <Step>
        ...
        import { lastLoginMethodClient } from "reign-auth/client/plugins" // [!code highlight]
...
<Steps>
    <Step>
        ...
        ```ts title="auth.ts"
        import { reignAuth } from "reign-auth"
        import { lastLoginMethod } from "reign-auth/plugins"
...
export const auth = reignAuth({
    plugins: [
        lastLoginMethod({
            ...
            cookieName: "reign-auth.last_used_login_method", // Default: "reign-auth.last_used_login_method"
...
```ts title="auth-client.ts"
import { createAuthClient } from "reign-auth/client"
import { lastLoginMethodClient } from "reign-auth/client/plugins"
...
The plugin automatically inherits cookie settings from Reign Auth's centralized cookie system. This solves the problem where the last login method wouldn't persist across:

- **Cross-subdomain setups**: `auth.example.com` → `app.example.com`
- **Cross-origin setups**: `api.company.com` → `app.different.com`

When you enable `crossSubDomainCookies` or `crossOriginCookies` in your Reign Auth config, the plugin will automatically use the same domain, secure, and sameSite settings as your session cookies, ensuring consistent behavior across your application.
...
When using Reign Auth with Expo, make sure to import the client plugin from `@reign-auth/expo/plugins` rather than from `reign-auth/plugins/client`. This ensures the last login method is stored correctly using the configured storage.

```ts
import { createAuthClient } from "reign-auth/react"
import { expoClient } from "@reign-auth/expo"
import { lastLoginMethodClient } from "@reign-auth/expo/plugins" // [!code highlight]
