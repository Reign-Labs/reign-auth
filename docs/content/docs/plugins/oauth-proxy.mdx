---
title: OAuth Proxy
description: OAuth Proxy plugin for Reign Auth
---
...
<Steps>
  <Step>
    ...
    import { oAuthProxy } from "reign-auth/plugins" // [!code highlight]
...
<Callout type="info">
All environments participating in the OAuth proxy flow must share the same encryption key. By default, the plugin uses `REIGN_AUTH_SECRET`. To avoid sharing your main secret across environments, you can provide a dedicated `secret` option (see [Options](#options)).
</Callout>
...
**productionURL**: The URL of your production server. If this value matches the `baseURL` in your auth config, requests will not be proxied. Defaults to the `REIGN_AUTH_URL` environment variable.
...
**secret**: A dedicated secret used for encrypting and decrypting data during the OAuth proxy flow. When set, this is used **instead of** the global `REIGN_AUTH_SECRET`, limiting the blast radius if the key is shared across environments — a leaked proxy secret cannot forge sessions or decrypt other data protected by the main secret. All environments participating in the proxy flow must share the same `secret` value.
