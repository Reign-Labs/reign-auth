...
import { organization } from "reign-auth/plugins" // [!code highlight]
...
<Step>
    ...
    import { organizationClient } from "reign-auth/client/plugins" // [!code highlight]
...
You can customize organization operations using hooks that run before and after various organization-related activities. Reign Auth provides two ways to configure hooks:
...
For member invitation to work we first need to provide `sendInvitationEmail` to the `reign-auth` instance. This function is responsible for sending the invitation email to the user.
...
<Steps>
    ...
    <Callout type="warning">
      To keep bundle sizes small, make sure to import from `reign-auth/plugins/access` instead of `reign-auth/plugins`.
    </Callout>
    </Step>
    ...
    ```ts title="permissions.ts"
    import { createAccessControl } from "reign-auth/plugins/access";
    import { defaultStatements, adminAc } from 'reign-auth/plugins/organization/access'
...
```ts title="auth-client.ts"
import { createAuthClient } from "reign-auth/client"
import { organizationClient, inferOrgAdditionalFields } from "reign-auth/client/plugins"
...
Starting with [Reign Auth v1.3](https://github.com/Reign-Labs/reign-auth/releases/tag/v1.3.0), you can easily add custom fields to the `organization`, `invitation`, `member`, and `team` tables.
