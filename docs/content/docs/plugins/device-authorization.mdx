...
You can test the device authorization flow right now using the Reign Auth CLI:

```package-install
npx reign-auth login
```

This will demonstrate the complete device authorization flow by:
1. Requesting a device code from the Reign Auth demo server
...
<Callout type="info">
  The CLI login command is a demo feature that connects to the Reign Auth demo server to showcase the device authorization flow in action.
</Callout>
...
<Steps>
    <Step>
        ...
        import { deviceAuthorization } from "reign-auth/plugins"; // [!code highlight]
    ...
    <Step>
        ...
        import { deviceAuthorizationClient } from "reign-auth/client/plugins"; // [!code highlight]
...
```ts title="auth-client.ts"
import { createAuthClient } from "reign-auth/client";
import { deviceAuthorizationClient } from "reign-auth/client/plugins";
...
async function authenticateCLI() {
  console.log("🔐 Reign Auth Device Authorization Demo");
