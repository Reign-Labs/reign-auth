...
The **Captcha Plugin** integrates bot protection into your Reign Auth system by adding captcha verification for key endpoints. This plugin ensures that only human users can perform actions like signing up, signing in, or resetting passwords. The following providers are currently supported:
...
<Steps>
  <Step>
    ...
    import { captcha } from "reign-auth/plugins"; // [!code highlight]
