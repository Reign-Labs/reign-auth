...
The i18n plugin allows you to translate error messages returned by Reign Auth based on the user's locale. It supports multiple locale detection strategies including HTTP headers, cookies, session data, and custom callbacks.

Reign Auth already provides English error messages by default, so you only need to provide translations for other languages.
...
A dictionary of translations keyed by locale code. Each locale contains a mapping of error codes to translated messages. Since Reign Auth already provides English messages, you typically only need to provide translations for other languages.
