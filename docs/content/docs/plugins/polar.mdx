---
title: Polar
description: Reign Auth Plugin for Payment and Checkouts using Polar
---

[Polar](https://polar.sh) is a developer first payment infrastructure. Out of the box it provides a lot of developer first integrations for payments, checkouts and more. This plugin helps you integrate Polar with Reign Auth to make your auth + payments flow seamless.
...
```bash
pnpm add reign-auth @polar-sh/reign-auth @polar-sh/sdk
```
...
import { polar, checkout, portal, usage, webhooks } from "@polar-sh/reign-auth";
...
### Configuring ReignAuth Client

You will be using the ReignAuth Client to interact with the Polar functionalities.

```typescript
import { createAuthClient } from "reign-auth/react";
import { polarClient } from "@polar-sh/reign-auth/client";

// This is all that is needed
// All Polar plugins, etc. should be attached to the server-side ReignAuth config
...
```typescript
import { polar, checkout } from "@polar-sh/reign-auth";
...
When checkouts are enabled, you're able to initialize Checkout Sessions using the checkout-method on the ReignAuth Client. This will redirect the user to the Product Checkout.
...
```typescript
import { polar, checkout, portal } from "@polar-sh/reign-auth";
...
The portal-plugin gives the ReignAuth Client a set of customer management methods, scoped under `authClient.customer`.
...
```typescript
import { polar, checkout, portal, usage } from "@polar-sh/reign-auth";
...
```typescript
import { polar, webhooks } from "@polar-sh/reign-auth";
