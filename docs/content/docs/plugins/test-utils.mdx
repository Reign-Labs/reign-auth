...
The Test Utils plugin provides helpers for writing integration and E2E tests against Reign Auth. It includes factories, database helpers, authentication helpers, and OTP capture functionality.
...
<Steps>
  <Step>
    ...
    import { testUtils } from "reign-auth/plugins" // [!code highlight]
...
- `name` - Cookie name (e.g., "reign-auth.session_token")
- `value` - Cookie value
...
```ts title="auth.ts"
import { reignAuth } from "reign-auth"
import { testUtils, emailOTP } from "reign-auth/plugins"
...
import type { TestHelpers } from "reign-auth/plugins"
