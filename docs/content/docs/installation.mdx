---
title: Installation
description: Learn how to configure Reign Auth in your project.
---
...
Let's start by adding Reign Auth to your project:
...
<Callout type="info">
If you're using a separate client and server setup, make sure to install Reign Auth in both parts of your project.
</Callout>
</Step>
...
<Callout type="info">
Need to rotate your secret later? You can use `REIGN_AUTH_SECRETS` (plural) to roll over to a new secret without invalidating existing data. See the [`secrets` option](/docs/reference/options#secrets) for details.
</Callout>

2. **Set Base URL**

```txt title=".env"
REIGN_AUTH_URL=http://localhost:3000 # Base URL of your app
```
...
And in this file, import Reign Auth and create your auth instance. Make sure to export the auth instance with the variable name `auth` or as a `default` export.
...
Reign Auth requires a database to store user data.
You can easily configure Reign Auth to use SQLite, PostgreSQL, or MySQL, and more!

<Callout>
    You can also configure Reign Auth to work in a stateless mode if you don't configure a database. See [Stateless Session Management](/docs/concepts/session-management#stateless-session-management) for more information. Note that most plugins will require a database.
</Callout>
...
<Tabs items={["drizzle", "prisma", "mongodb"]}>
    ...
    ```ts title="auth.ts"
    import { reignAuth } from "reign-auth";
    import { mongodbAdapter } from "reign-auth/adapters/mongodb";
...
<Step>
    ...
    Reign Auth includes a CLI tool to help manage the schema required by the library.
...
Configure the authentication methods you want to use. Reign Auth comes with built-in support for email/password, and social sign-on providers.
...
<Callout>
  Reign Auth supports any backend framework with standard Request and Response
...
<Tabs items={["next-js-app-router", "next-js-pages-router", "nuxt", "svelte-kit", "react-router", "solid-start", "hono", "cloudflare-workers", "express", "elysia", "tanstack-start", "expo"]} defaultValue="next-js-app-router">
    ...
    <Callout type="info">
    **Node.js AsyncLocalStorage Support**: Reign Auth uses AsyncLocalStorage for async context tracking. To enable this in Cloudflare Workers, add the `nodejs_compat` flag to your `wrangler.toml`:
    ...
    // Mount express json middleware after Reign Auth handler
    // or only apply it to routes that don't interact with Reign Auth
    app.use(express.json());

    app.listen(port, () => {
        console.log(`Reign Auth app listening on port ${port}`);
    });
    ```
    ...
    const reignAuthView = (context: Context) => {
        const REIGN_AUTH_ACCEPT_METHODS = ["POST", "GET"]
        // validate request method
        if(REIGN_AUTH_ACCEPT_METHODS.includes(context.request.method)) {
    ...
    const app = new Elysia().all("/api/auth/*", reignAuthView).listen(3000);
    ...
    <Callout type="info">
    When you call functions that need to set cookies (like `signInEmail` or `signUpEmail`), you'll need to handle cookie setting for TanStack Start. Reign Auth provides a `tanstackStartCookies` plugin to automatically handle this for you.
...
The client-side library helps you interact with the auth server. Reign Auth comes with a client for all the popular web frameworks, including vanilla JavaScript.

1. Import `createAuthClient` from the package for your framework (e.g., "reign-auth/react" for React).
...
<Tabs items={["react", "vue", "svelte", "solid",
  "vanilla"]} defaultValue="react">
    ...
    <Tab value="svelte" title="lib/auth-client.ts">
            ```ts  title="lib/auth-client.ts"
            import { createAuthClient } from "reign-auth/svelte"
            export const authClient = createAuthClient({
                /** The base URL of the server (optional if you're using the same domain) */ // [!code highlight]
                baseURL: "http://localhost:3000" // [!code highlight]
            })
...
<Step>
### 🎉 That's it!
That's it! You're now ready to use reign-auth in your application. Continue to [basic usage](/docs/basic-usage) to learn how to use the auth instance to sign in users.
</Step>
</Steps>
