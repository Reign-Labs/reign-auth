...
Reign Auth provides support for any social provider that implements the OAuth2 protocol or OpenID Connect (OIDC) flows through the [Generic OAuth Plugin](/docs/plugins/generic-oauth). You can use pre-configured helper functions for popular providers like Auth0, Keycloak, Okta, Microsoft Entra ID, and Slack, or manually configure any OAuth provider.
...
<Steps>
  ...
  <Step>
    ...
    import { genericOAuthClient } from "reign-auth/client/plugins"
...
```ts title="auth.ts"
import { reignAuth } from "reign-auth"
import { genericOAuth } from "reign-auth/plugins"
...
Reign Auth provides pre-configured helper functions for popular OAuth providers. Here's an example using Slack:

```ts title="auth.ts"
import { reignAuth } from "reign-auth"
import { genericOAuth, slack } from "reign-auth/plugins"
