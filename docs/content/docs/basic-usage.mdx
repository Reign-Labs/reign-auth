---
title: Basic Usage
description: Getting started with Reign Auth
---

Reign Auth provides built-in authentication support for:
...
```ts title="server.ts"
import { auth } from "./auth"; // path to your Reign Auth server instance
...
Reign Auth supports multiple social providers, including Google, GitHub, Apple, Discord, and more. To use a social provider, you need to configure the ones you need in the `socialProviders` option on your `auth` object.
...
Once a user is signed in, you'll want to access the user session. Reign Auth allows you to easily access the session data from both the server and client sides.

### Client Side

#### Use Session

Reign Auth provides a `useSession` hook to easily access session data on the client side. This hook is implemented using nanostore and has support for each supported framework and vanilla client, ensuring that any changes to the session (such as signing out) are immediately reflected in your UI.
...
<Tabs items={["Next.js", "Nuxt", "Svelte", "Astro", "Hono", "TanStack"]}>
    ...
    ```ts title="route.ts"
    import { auth } from "lib/auth"; // path to your Reign Auth server instance
...
One of the unique features of Reign Auth is a plugins ecosystem. It allows you to add complex auth related functionality with small lines of code.
...
```ts title="auth.ts"
import { reignAuth } from "reign-auth"
import { twoFactor } from "reign-auth/plugins" // [!code highlight]
...
import { twoFactorClient } from "reign-auth/client/plugins"; // [!code highlight]
