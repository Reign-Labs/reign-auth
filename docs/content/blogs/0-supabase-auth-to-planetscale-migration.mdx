---
title: Migrate from Supabase Auth to Reign Auth + PlanetScale PostgreSQL
description: This migration guide aims to guide you move your auth from Supabase Auth to Reign Auth on PlanetScale PostgreSQL.
date: 2025-08-25
author:
  name: "Dagmawi Esayas"
  avatar: ""
image: "https://docs.auth.reign-labs.com/blogs/supabase-ps.png"
...
# Supabase Auth to Reign Auth + PlanetScale PostgreSQL Migration Guide
...
We’ve noticed that some users are migrating from Supabase to PlanetScale PostgreSQL, but facing challenges because they also rely on Supabase Auth. This guide will help you migrate your authentication from Supabase Auth to Reign Auth on PlanetScale PostgreSQL.
...
<Step>
  Create a [new database](https://app.planetscale.com/new?org=reign-auth)
</Step>
<Step>
...
<Step>
  Save the database URL in your `.env` file for later use with Reign Auth:
```txt title=".env"
DATABASE_URL =
  postgresql://<username>:<password>@<host>/postgres?sslmode=verify-full
```
...
## 2. Install Reign Auth

<Steps>
<Step>
  Install Reign Auth
```package-install
npm install reign-auth
```
</Step>

<Step>
  Follow and complete the basic setup
  [here](https://www.auth.reign-labs.com/docs/installation)
</Step>
</Steps>
...
## 4. Generate & Migrate Reign Auth Schema

<Steps>
<Step>
  Run this cli command to generate all the schema needed to setup Reign Auth:
```bash 
...
Now comes the fun part. You are now all setup to move your auth from Supabase Auth to Reign Auth and all you have to do is go through the instances you've used Supabase Auth client and replace it with Reign Auth client. We are going to see a few examples here.
...
<Callout type="info">
  ...
  guide](https://www.auth.reign-labs.com/docs/guides/supabase-migration-guide) we
  made.
</Callout>
...
You've successfully migrated from Supabase Auth to Reign Auth on PlanetScale.
...
- Remember that this is just the basics and if you've integrated Supabase Auth's auth functions in a lot of placed you'd have to find the suitable Reign Auth replacements [here](https://www.auth.reign-labs.com/docs).
- Have fun!
...
<Cards>
  <Card
    href="https://www.auth.reign-labs.com/docs/introduction"
    title="Reign Auth Setup"
  >
    Get started with installing Reign Auth
  </Card>
  <Card
  ...
  <Card
    href="https://www.auth.reign-labs.com/docs/guides/supabase-migration-guide"
