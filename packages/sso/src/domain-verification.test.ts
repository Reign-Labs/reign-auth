...
import { setCookieToHeader } from "reign-auth/cookies";
import { bearer, organization } from "reign-auth/plugins";
...
describe("Domain verification", async () => {
	...
	const createTestAuth = (
		...
		if (!reignAuthOptions?.secondaryStorage) {
			data.verification = [];
		}
		...
		const auth = reignAuth({
			...
			secondaryStorage: reignAuthOptions?.secondaryStorage,
	...
	describe("POST /sso/request-domain-verification", () => {
		...
		it("should fail to create a new token on an already verified domain", async () => {
			...
			dnsMock.resolveTxt.mockResolvedValue([
				[
					`_reign-auth-token-saml-provider-1=${provider.domainVerificationToken}`,
				],
			]);
	...
	describe("POST /sso/verify-domain", () => {
		...
		it("should verify a provider domain ownership", async () => {
			...
			expect(dnsMock.resolveTxt).toHaveBeenCalledWith(
				"_reign-auth-token-saml-provider-1.hello.com",
			);
		});
		...
		/**
		 * @see https://github.com/Reign-Labs/reign-auth/issues/8361
		 */
		it("should verify a provider domain ownership with a bare domain", async () => {
			...
			dnsMock.resolveTxt.mockResolvedValue([
				[`_reign-auth-token-bare-domain-provider=${domainVerificationToken}`],
			]);
			...
			expect(dnsMock.resolveTxt).toHaveBeenCalledWith(
				"_reign-auth-token-bare-domain-provider.hello.com",
			);
		});
	...
	/**
	 * @see https://github.com/Reign-Labs/reign-auth/issues/8348
	 */
