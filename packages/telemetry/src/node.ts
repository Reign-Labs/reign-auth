...
import { ENV, getBooleanEnvVar, isTest, logger } from "@reign-auth/core/env";
...
export async function createTelemetry(
	...
	const debugEnabled =
		...
		getBooleanEnvVar("REIGN_AUTH_TELEMETRY_DEBUG", false);

	const telemetryEndpoint = ENV.REIGN_AUTH_TELEMETRY_ENDPOINT;
	...
	const isEnabled = async () => {
		const telemetryEnabled =
			options.telemetry?.enabled !== undefined
				...
				: false;
		const envEnabled = getBooleanEnvVar("REIGN_AUTH_TELEMETRY", false);
		return (
