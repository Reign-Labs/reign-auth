...
beforeEach(() => {
	vi.resetModules();
	process.env.REIGN_AUTH_TELEMETRY = "";
	process.env.REIGN_AUTH_TELEMETRY_DEBUG = "";
});
...
describe("telemetry", () => {
	...
	it("does not publish when disabled via env", async () => {
		process.env.REIGN_AUTH_TELEMETRY = "false";
		let event: TelemetryEvent | undefined;
		const track = vi.fn().mockImplementation(async (e) => {
			event = e;
		});
	...
	it("returns noop publisher when REIGN_AUTH_TELEMETRY_ENDPOINT is undefined", async () => {
		// Import betterFetch mock to check it's not called
		const { betterFetch } = await import("@reign-auth/fetch");

		// Clear any previous calls to the mock
		vi.mocked(betterFetch).mockClear();

		// Ensure the environment variable is not set
		vi.stubEnv("REIGN_AUTH_TELEMETRY_ENDPOINT", undefined);
