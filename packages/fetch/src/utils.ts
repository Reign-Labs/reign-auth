...
export function getBody(options?: BetterFetchOption) {
	...
	const _headers = options?.headers instanceof Headers ? options.headers : new Headers(options?.headers as HeadersInit | undefined);
	if (
		_headers.has("content-type") &&
		_headers.get("content-type") === "application/x-www-form-urlencoded"
	) {
