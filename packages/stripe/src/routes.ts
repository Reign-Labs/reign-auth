...
import type { GenericEndpointContext, User } from "reign-auth";
import { HIDE_METADATA } from "reign-auth";
import { getSessionFromCtx, originCheck } from "reign-auth/api";
import type { Organization } from "reign-auth/plugins/organization";
 ...
 *
 * @see [Read our docs to learn more.](https://auth.reign-labs.com/docs/plugins/stripe#api-method-subscription-upgrade)
 */
		...
		async (ctx) => {
			...
			if (activeSubscription && customerId) {
				...
				if (activeSubscription.schedule) {
					...
					if (
						existingSchedule &&
						existingSchedule.metadata?.source === "@reign-auth/stripe"
					) {
				...
				if (ctx.body.scheduleAtPeriodEnd) {
					...
					await client.subscriptionSchedules
						.update(schedule.id, {
							metadata: { source: "@reign-auth/stripe" },
 ...
 *
 * @see [Read our docs to learn more.](https://auth.reign-labs.com/docs/plugins/stripe#api-method-subscription-cancel)
 */
 ...
 *
 * @see [Read our docs to learn more.](https://auth.reign-labs.com/docs/plugins/stripe#api-method-subscription-list)
 */
