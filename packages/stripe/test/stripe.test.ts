import { runWithEndpointContext } from "@reign-auth/core/context";
import type { Auth, User } from "reign-auth";
import { memoryAdapter } from "reign-auth/adapters/memory";
...
describe("stripe", () => {
	...
	/**
	 * @see https://github.com/Reign-Labs/reign-auth/issues/7959
	 */
	...
	/**
	 * @see https://github.com/Reign-Labs/reign-auth/issues/6863
	 */
	...
	it("should not release schedules created outside the plugin", async () => {
		...
		// Schedule created externally (no @reign-auth/stripe metadata)
		mockStripe.subscriptionSchedules.list.mockResolvedValueOnce({
			data: [
				{
	...
	describe("subscriptionSuccess - checkoutSessionId flow", () => {
		...
		/**
		 * @see https://github.com/Reign-Labs/reign-auth/issues/8255
		 */
		it("should replace {CHECKOUT_SESSION_ID} placeholder in callbackURL with actual session ID", async () => {
			...
			// Reign Auth should replace it with the actual session ID before redirecting.
			const callbackURL =
