		...
		await client.signIn.email(
			...
			},
			{
				onResponse(context) {
					...
					expect(setCookie).toContain("reign-auth");
				},
			},
		);
	});
...
describe("cookie configuration", () => {
	it("should return correct cookie options based on configuration", async () => {
		const options = {
			baseURL: "https://example.com",
			database: {} as ReignAuthOptions["database"],
			advanced: {
...
describe("cookie-utils stripSecureCookiePrefix", () => {
	...
	it("should handle cookie names with dots and special characters", () => {
		const cookieName = `${SECURE_COOKIE_PREFIX}reign-auth.session_token`;
		const result = stripSecureCookiePrefix(cookieName);
		expect(result).toBe("reign-auth.session_token");
	});
});
...
describe("getSessionCookie", async () => {
	...
	it("should work with cross-realm Headers-like object (instanceof fails)", async () => {
		...
		// See: https://github.com/Reign-Labs/reign-auth/pull/1838
	...
	describe.each([".", "-"])("with '%s' separator", (separator) => {
		describe.each([
			...
			it.each([
				...
				[{ cookieName: "my_token" }, "reign-auth", "my_token"],
				[
		...
		await client.signIn.email(
			...
			},
			{
				onSuccess(c) {
					...
					const sessionTokenCookie = parsed.get("reign-auth.session_token")!;
					expect(sessionTokenCookie).toBeDefined();
					expect(sessionTokenCookie["max-age"]).toBeUndefined();

					const sessionDataCookie = parsed.get("reign-auth.session_data")!;
...
describe("Cookie Cache Field Filtering", () => {
	...
	it("should return null for invalid JWT token", async () => {
		...
		headers.set("cookie", "reign-auth.session_data=invalid.jwt.token");
...
describe("parse cookies", () => {
	it("should parse cookies into key-value map", () => {
		const cookieHeader =
			"reign-auth.session_token=session-token.signature; reign-auth.session_data=session-data.signature";

		const parsedCookies = parseCookies(cookieHeader);

		expect(parsedCookies.get("reign-auth.session_token")).toBe(
			"session-token.signature",
		);
		expect(parsedCookies.get("reign-auth.session_data")).toBe(
