...
import { env, isProduction } from "@reign-auth/core/env";
import { ReignAuthError } from "@reign-auth/core/error";
import { filterOutputFields } from "@reign-auth/core/utils/db";
import { safeJSONParse } from "@reign-auth/core/utils/json";
import { base64Url } from "@reign-auth/utils/base64";
import { binary } from "@reign-auth/utils/binary";
import { createHMAC } from "@reign-auth/utils/hmac";
...
export function createCookieGetter(options: ReignAuthOptions) {
	const baseURLString =
	...
	function createCookie(
		...
		const prefix = options.advanced?.cookiePrefix || "reign-auth";
		const name =
		...
		} satisfies ReignAuthCookie;
	}
	return createCookie;
}

export function getCookies(options: ReignAuthOptions) {
...
export type EligibleCookies = (string & {}) | (keyof ReignAuthCookies & {});
...
export const getSessionCookie = (
	...
	const { cookieName = "session_token", cookiePrefix = "reign-auth" } =
		config || {};
...
>(
	...
	const { cookieName = "session_data", cookiePrefix = "reign-auth" } =
		config || {};
	const name =
	...
	if (sessionData) {
		const secret = config?.secret || env.REIGN_AUTH_SECRET;
		if (!secret) {
			throw new ReignAuthError(
				"getCookieCache requires a secret to be provided. Either pass it as an option or set the REIGN_AUTH_SECRET environment variable",
			);
		}
