...
describe("multi-session", async () => {
			...
			},
			{
				onResponse(context) {
					...
					const sessionToken = setCookies
						.get("reign-auth.session_token")
						?.value.split(".")[0];
					const multiSession = setCookies.get(
						`reign-auth.session_token_multi-${sessionToken?.toLowerCase()}`,
					)?.value;
					expect(sessionToken).not.toBe(null);
					expect(multiSession).not.toBe(null);
					expect(multiSession).toContain(sessionToken);
					expect(setCookieString).toContain("reign-auth.session_token_multi-");
				},
	...
	it("should set active session when only multi-session cookies are present", async () => {
		...
		const multiOnlyCookieHeader = existingCookieHeader
			...
			.filter(Boolean)
			.filter((cookie) => !cookie.startsWith("reign-auth.session_token="))
			.join("; ");
	...
	it("should revoke a session and set the next active", async () => {
		...
		await client.signUp.email(testUser3, {
			onSuccess: (ctx) => {
				...
				expect(header).toContain("reign-auth.session_token");
				const cookies = parseSetCookieHeader(header || "");
				token =
					cookies.get("reign-auth.session_token")?.value.split(".")[0] || "";
			},
		});
		...
		await client.signIn.email(
			...
			},
			{
				...
				onResponse(context) {
					...
					const oldCookieName = `reign-auth.session_token_multi-${firstSessionToken.toLowerCase()}`;
	...
	it("should reject forged multi-session cookies on sign-out", async () => {
		...
		const forgedCookieName = `reign-auth.session_token_multi-${victimSessionToken.toLowerCase()}`;
