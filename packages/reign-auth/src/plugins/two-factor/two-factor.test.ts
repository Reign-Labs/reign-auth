import { createOTP } from "@reign-auth/utils/otp";
...
describe("two factor", async () => {
		...
		const res = await client.signIn.email({
			...
			fetchOptions: {
				onResponse(context) {
					...
					expect(parsed.get("reign-auth.session_token")?.value).toBe("");
					expect(parsed.get("reign-auth.session_data")?.value).toBe("");
					expect(parsed.get("reign-auth.two_factor")?.value).toBeDefined();
					expect(parsed.get("reign-auth.dont_remember")?.value).toBeDefined();
					headers.append(
						"cookie",
						`reign-auth.two_factor=${
							parsed.get("reign-auth.two_factor")?.value
						}`,
					);
					headers.append(
						"cookie",
						`reign-auth.dont_remember=${
							parsed.get("reign-auth.dont_remember")?.value
						}`,
					);
				},
			},
		...
		const verifyRes = await client.twoFactor.verifyOtp({
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					expect(parsed.get("reign-auth.session_token")?.value).toBeDefined();
					// max age should be undefined because we are not using remember me
					expect(
						parsed.get("reign-auth.session_token")?.["max-age"],
					).not.toBeDefined();
				},
			},
		});
		...
		const verifyRes = await client.twoFactor.verifyOtp({
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					expect(
						parsed.get("reign-auth.session_token")?.value,
					).not.toBeDefined();
				},
			},
		});
	...
	it("should allow sign in with backup code", async () => {
		...
		await client.signIn.email({
			...
			fetchOptions: {
				onSuccess(context) {
					...
					const token = parsed.get("reign-auth.session_token")?.value;
					expect(token).toBe("");
					headers.append(
						"cookie",
						`reign-auth.two_factor=${
		...
		const token = parsedCookies.get("reign-auth.session_token")?.value;
		...
		await client.twoFactor.sendOtp({
			fetchOptions: {
				...
				onSuccess(context) {
					...
					headers.append(
						"cookie",
						`reign-auth.otp.counter=${
							parsed.get("reign-auth.otp_counter")?.value
						}`,
					);
				},
			},
		...
		await client.twoFactor.verifyOtp({
			...
			fetchOptions: {
				...
				onSuccess(context) {
					...
					expect(parsed.get("reign-auth.trust_device")?.value).toBeDefined();
					newHeaders.set(
						"cookie",
						`reign-auth.trust_device=${
							parsed.get("reign-auth.trust_device")?.value
						}`,
					);
				},
			},
...
describe("two factor auth API", async () => {
	...
	it("should request second factor", async () => {
		...
		const twoFactorCookie = parsed.get("reign-auth.two_factor");
		expect(twoFactorCookie).toBeDefined();
		const sessionToken = parsed.get("reign-auth.session_token");
		...
		await client.twoFactor.verifyBackupCode({
			...
			fetchOptions: {
				...
				onSuccess(context) {
					...
					sessionToken = parsed.get("reign-auth.session_token")?.value || "";
				},
			},
		});
...
describe("trust device server-side validation", async () => {
	...
	it("should force 2FA when server-side trust record is expired", async () => {
		...
		trustHeaders.set(
			"cookie",
			`reign-auth.trust_device=${trustDeviceCookieValue}`,
		);
		...
		const clearedTrustCookie = signIn2Parsed.get("reign-auth.trust_device");
	...
	it("should revoke trust device when disabling 2FA", async () => {
		...
		const clearedCookie = disableParsed.get("reign-auth.trust_device");
...
describe("trustDeviceMaxAge", async () => {
	...
	it("should use custom trustDeviceMaxAge for the trust device cookie", async () => {
		...
		const trustDeviceCookie = parsed.get("reign-auth.trust_device");
...
/**
 * @see https://github.com/Reign-Labs/reign-auth/issues/8424
 */
