		...
		await client.getSession({
			fetchOptions: {
				...
				onResponse(context) {
					...
					expect(joined).toContain("reign-auth.session_token");
					expect(joined).toContain("reign-auth.session_data");
				},
			},
		});
	});
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					const tokenCookie = setCookies.find((c) =>
						c.includes("reign-auth.session_token"),
					);
					const dataCookie = setCookies.find((c) =>
						c.includes("reign-auth.session_data"),
					);
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					for (const cookie of setCookies) {
						...
						// None of those attributes should be another "reign-auth." cookie
						const reignAuthEntries = cookieNames.filter((n) =>
							n.startsWith("reign-auth."),
						);
						expect(reignAuthEntries).toHaveLength(1);
					}
				},
			},
		});
