...
describe("oauth2", async () => {
	...
	it("should delete state cookie with path attribute", async () => {
		...
		const stateCookie = cookies.get("reign-auth.state");
			...
			},
			{
				onSuccess(context) {
					...
					headers.set(
						"cookie",
						`reign-auth.state=${
							parsedSetCookie.get("reign-auth.state")?.value
						}; reign-auth.pk_code_verifier=${
							parsedSetCookie.get("reign-auth.pk_code_verifier")?.value
						}`,
					);
				},
			},
