import type { BaseURLConfig, DynamicBaseURLConfig } from "@reign-auth/core";
...
function assertHasProtocol(url: string): void {
	try {
		...
		if (error instanceof ReignAuthError) {
			throw error;
		}
		throw new ReignAuthError(
			`Invalid base URL: ${url}. Please provide a valid base URL.`,
...
export function getBaseURL(
	...
	if (loadEnv !== false) {
		const fromEnv =
			env.REIGN_AUTH_URL ||
			env.NEXT_PUBLIC_REIGN_AUTH_URL ||
			env.PUBLIC_REIGN_AUTH_URL ||
			env.NUXT_PUBLIC_REIGN_AUTH_URL ||
 ...
 * @throws ReignAuthError if host is not in allowedHosts and no fallback is set
 */
