...
describe("base context creation", () => {
	const initBase = async (options: Partial<ReignAuthOptions> = {}) => {
		const opts: ReignAuthOptions = {
	...
	it("should infer BASE_URL from env", async () => {
		vi.stubEnv("REIGN_AUTH_URL", "http://localhost:5147");
	...
	describe("secret management", () => {
		...
		it("should use REIGN_AUTH_SECRET from env", async () => {
			vi.stubEnv(
				"REIGN_AUTH_SECRET",
				"reign-auth-secret-that-is-long-enough-for-validation",
			);
			const opts: ReignAuthOptions = {};
			const adapter = await getAdapter(opts);
			const getDatabaseType = () => "memory";
	...
	describe("app name", () => {
		it("should use default app name", async () => {
			const res = await initBase({});
			expect(res.appName).toBe("Reign Auth");
		});
	...
	describe("trusted origins - environment variables", () => {
		it("should include origins from REIGN_AUTH_TRUSTED_ORIGINS env", async () => {
			vi.stubEnv(
				"REIGN_AUTH_TRUSTED_ORIGINS",
	...
	describe("secret validation", () => {
		...
		it("should throw error when default secret is set in production environment", async () => {
			...
			vi.doMock("@reign-auth/core/env", async () => {
				const actual = await vi.importActual("@reign-auth/core/env");
				return {
					...actual,
					isProduction: true,
					isTest: () => false,
			...
			const initBaseProduction = async (
				options: Partial<ReignAuthOptions> = {},
			) => {
				const opts: ReignAuthOptions = {
					baseURL: "http://localhost:3000",
					...options,
			...
			vi.doUnmock("@reign-auth/core/env");
			vi.resetModules();
