import type { SecretConfig } from "@reign-auth/core";
import type { createLogger } from "@reign-auth/core/env";
	...
	const entries = envValue.split(",").map((entry) => {
		...
		if (colonIdx === -1) {
			throw new ReignAuthError(
				`Invalid REIGN_AUTH_SECRETS entry: "${entry}". Expected format: "<version>:<secret>"`,
			);
		}
		const version = parseInt(entry.slice(0, colonIdx), 10);
		if (!Number.isInteger(version) || version < 0) {
			throw new ReignAuthError(
				`Invalid version in REIGN_AUTH_SECRETS: "${entry.slice(0, colonIdx)}". Version must be a non-negative integer.`,
			);
		}
		const value = entry.slice(colonIdx + 1).trim();
		if (!value) {
			throw new ReignAuthError(
				`Empty secret value for version ${version} in REIGN_AUTH_SECRETS.`,
			);
		}
...
export function validateSecretsArray(
	...
	if (current.value.length < 32) {
		logger.warn(
			`[reign-auth] Warning: the current secret (version ${current.version}) should be at least 32 characters long for adequate security.`,
		);
	}
	const entropy = estimateEntropy(current.value);
	if (entropy < 120) {
		logger.warn(
			"[reign-auth] Warning: the current secret appears low-entropy. Use a randomly generated secret for production.",
		);
	}
}
