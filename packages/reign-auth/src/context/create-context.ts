...
import { getReignAuthVersion } from "@reign-auth/core/context";
import { getAuthTables } from "@reign-auth/core/db";
import type { DBAdapter } from "@reign-auth/core/db/adapter";
import { createLogger, env, isProduction, isTest } from "@reign-auth/core/env";
import { ReignAuthError } from "@reign-auth/core/error";
import type { OAuthProvider } from "@reign-auth/core/oauth2";
import type { SocialProviders } from "@reign-auth/core/social-providers";
import { socialProviders } from "@reign-auth/core/social-providers";
import { generateId } from "@reign-auth/core/utils/id";
import { createTelemetry } from "@reign-auth/telemetry";
 ...
 * Throws ReignAuthError if the secret is invalid.
...
function validateSecret(
	...
	if (isDefaultSecret && isProduction) {
		throw new ReignAuthError(
			"You are using the default secret. Please set `REIGN_AUTH_SECRET` in your environment variables or pass `secret` in your auth config.",
		);
	}

	if (!secret) {
		throw new ReignAuthError(
			"REIGN_AUTH_SECRET is missing. Set it in your environment or pass `secret` to reignAuth({ secret }).",
		);
	}

	if (secret.length < 32) {
		logger.warn(
			`[reign-auth] Warning: your REIGN_AUTH_SECRET should be at least 32 characters long for adequate security. Generate one with \`npx auth secret\` or \`openssl rand -base64 32\`.`,
		);
	}
	...
	if (entropy < 120) {
		logger.warn(
			"[reign-auth] Warning: your REIGN_AUTH_SECRET appears low-entropy. Use a randomly generated secret for production.",
		);
	}
}

export async function createAuthContext<Options extends ReignAuthOptions>(
	adapter: DBAdapter,
	options: Options,
	...
	if (!baseURL && !isDynamicConfig) {
		logger.warn(
			`[reign-auth] Base URL could not be determined. Please set a valid base URL using the baseURL config option or the REIGN_AUTH_URL environment variable. Without this, callbacks and redirects may not work correctly.`,
		);
	}
	...
	if (
		...
		logger.error(
			`[reign-auth] Misconfiguration detected.
You are using the memory DB with generateId: false.
This will cause no id to be generated for any model.
Most of the features of Reign Auth will not work correctly.`,
		);
	}

	const secretsArray =
		options.secrets ?? parseSecretsEnv(env.REIGN_AUTH_SECRETS);

	const legacySecret =
		options.secret || env.REIGN_AUTH_SECRET || env.AUTH_SECRET || "";
	...
	const ctx: AuthContext = {
		appName: options.appName || "Reign Auth",
		baseURL: baseURL || "",
		version: getReignAuthVersion(),
		...
		sessionConfig: {
			...
			cookieRefreshCache: (() => {
				...
				if (isStateful && refreshCache) {
					logger.warn(
						"[reign-auth] `session.cookieCache.refreshCache` is enabled while `database` or `secondaryStorage` is configured. `refreshCache` is meant for stateless (DB-less) setups. Disabling `refreshCache` — remove it from your config to silence this warning.",
					);
					return false;
				}
