...
import { env } from "@reign-auth/core/env";
...
export async function runPluginInit(context: AuthContext) {
	...
	const pluginTrustedOrigins: NonNullable<
		ReignAuthOptions["trustedOrigins"]
	>[] = [];
	const dbHooks: {
		source: string;
		hooks: Exclude<ReignAuthOptions["databaseHooks"], undefined>;
	}[] = [];
	for (const plugin of plugins) {
		if (plugin.init) {
			const initPromise = plugin.init(context);
			let result: ReturnType<Required<ReignAuthPlugin>["init"]>;
...
export function getInternalPlugins(options: ReignAuthOptions) {
	const plugins: ReignAuthPlugin[] = [];
...
export async function getTrustedOrigins(
	...
	const envTrustedOrigins = env.REIGN_AUTH_TRUSTED_ORIGINS;
