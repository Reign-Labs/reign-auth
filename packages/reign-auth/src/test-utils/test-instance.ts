...
import type { SuccessContext } from "@reign-auth/fetch";
import { sql } from "kysely";
import { afterAll } from "vitest";
import { reignAuth } from "../auth/full";
...
export async function getTestInstance<
	O extends Partial<ReignAuthOptions>,
	C extends ReignAuthClientOptions,
>(
	options?: O | undefined,
	config?:
		| {
	...
	async function getPostgres() {
		...
		return new Kysely({
			dialect: new PostgresDialect({
				pool: new Pool({
					connectionString:
						"postgres://user:password@localhost:5432/reign_auth",
				}),
			}),
		});
	}
	...
	async function getMysql() {
		...
		return new Kysely({
			dialect: new MysqlDialect({
				pool: createPool("mysql://user:password@localhost:3306/reign_auth"),
			}),
		});
	}
	...
	async function mongodbClient() {
		...
		const db = await dbClient("mongodb://127.0.0.1:27017", "reign-auth");
		return db;
	}
	...
	const opts = {
		...
		secret: "reign-auth-secret-that-is-long-enough-for-validation-test",
		database:
			testWith === "postgres"
		...
		const { data } = await client.signIn.email({
			...
			fetchOptions: {
				...
				onSuccess(context) {
					...
					const signedCookie = cookies.get("reign-auth.session_token")?.value;
					headers.set("cookie", `reign-auth.session_token=${signedCookie}`);
				},
			},
		});
		return {
