...
import type { DBFieldAttribute, DBFieldType } from "@reign-auth/core/db";
import { getAuthTables } from "@reign-auth/core/db";
import {
	initGetFieldName,
	initGetModelName,
} from "@reign-auth/core/db/adapter";
import { createLogger } from "@reign-auth/core/env";
import type { KyselyDatabaseType } from "@reign-auth/kysely-adapter";
import { createKyselyAdapter } from "@reign-auth/kysely-adapter";
import type {
...
export async function getMigrations(config: ReignAuthOptions) {
	const reignAuthSchema = getSchema(config);
	...
	for (const [key, value] of Object.entries(reignAuthSchema)) {
	...
	function getType(field: DBFieldAttribute, fieldName: string) {
		...
		if (!(type in typeMap)) {
			throw new Error(
				`Unsupported field type '${String(type)}' for field '${fieldName}'. Allowed types are: string, number, boolean, date, string[], number[]. If you need to store structured data, store it as a JSON string (type: "string") or split it into primitive fields. See https://auth.reign-labs.com/docs/advanced/schema#additional-fields`,
			);
		}
	...
	// user-supplied strings for external tables not in the ReignAuth schema
