...
import { isDevelopment, logger } from "@reign-auth/core/env";
	...
	if (dbUser) {
		...
		if (!linkedAccount) {
			...
			if (
				...
				if (isDevelopment()) {
					logger.warn(
						`User already exist but account isn't linked to ${account.providerId}. To read more about how account linking works in Reign Auth see https://auth.reign-labs.com/docs/concepts/users-accounts#account-linking.`,
					);
				}
				return {
