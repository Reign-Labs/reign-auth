...
import type { User } from "@reign-auth/core/db";
import { APIError, BASE_ERROR_CODES } from "@reign-auth/core/error";
import { SocialProviderListEnum } from "@reign-auth/core/social-providers";
...
export const signInSocial = <O extends ReignAuthOptions>() =>
	createAuthEndpoint(
		"/sign-in/social",
		{
			method: "POST",
...
export const signInEmail = <O extends ReignAuthOptions>() =>
	createAuthEndpoint(
		"/sign-in/email",
		{
			method: "POST",
		...
		async (
			...
			if (!ctx.context.options?.emailAndPassword?.enabled) {
				ctx.context.logger.error(
					"Email and password is not enabled. Make sure to enable it in the options on you `auth.ts` file. Check `https://auth.reign-labs.com/docs/authentication/email-password` for more!",
				);
