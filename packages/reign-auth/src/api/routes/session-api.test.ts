		...
		await client.signIn.email(
			...
			},
			{
				onSuccess(context) {
					...
					const cookie = cookies.get("reign-auth.session_token");
			...
			const response = await client.getSession({
				fetchOptions: {
					...
					onSuccess(context) {
						...
						const maxAge = parsed.get("reign-auth.session_token")?.["max-age"];
		...
		await client.signIn.email(
			...
			},
			{
				onSuccess(context) {
					const header = context.response.headers.get("set-cookie");
					const cookies = parseSetCookieHeader(header || "");
					expect(cookies.get("reign-auth.session_token")).toMatchObject({
						value: expect.any(String),
						"max-age": 60 * 60 * 24 * 7,
						path: "/",
						httponly: true,
...
describe("cookie cache with JWT strategy", async () => {
	...
	it("should not allow tampering with the cookie", async () => {
		...
		headers.set("cookie", `reign-auth.session_data=${newJWT}`);
		headers.append("cookie", `reign-auth.session_token=${sessionCookie}`);
...
describe("cookie cache refreshCache", async () => {
	...
	/**
	 * @see https://github.com/Reign-Labs/reign-auth/issues/7994
	 */
		...
		await client.getSession({
			fetchOptions: {
				...
				onSuccess(context) {
					...
					sessionTokenMaxAge = parsed.get("reign-auth.session_token")?.[
						"max-age"
					];
				},
			},
