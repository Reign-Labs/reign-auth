...
describe("account", async () => {
	...
	/**
	 * @see https://github.com/Reign-Labs/reign-auth/issues/8345
	 */
	...
	it("should encrypt account cookie payload", async () => {
		...
		await client.$fetch("/callback/google", {
			...
			async onError(context) {
				...
				const parsed = parseSetCookieHeader(setCookie!);
				const accountData = parsed.get("reign-auth.account_data")?.value;

				expect(accountData).toBeDefined();
				expect(accountData!.startsWith("ey")).toBe(true);
				await expect(
					symmetricDecodeJWT(accountData!, ctx.secret, "reign-auth-account"),
				).resolves.toMatchObject({
					accessToken: "test",
					refreshToken: "test",
					providerId: "google",
