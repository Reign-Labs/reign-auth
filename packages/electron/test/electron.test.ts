...
import { createAuthMiddleware } from "@reign-auth/core/api";
...
import { parseSetCookieHeader } from "reign-auth/cookies";
import { generateRandomString } from "reign-auth/crypto";
...
describe("Electron", () => {
		...
		const { error } = await proxyClient.signUp.email(
			...
			},
			{
				...
				onResponse: async (ctx) => {
					...
					const redirectCookie = cookies.get(`reign-auth.electron`);
	...
	it("should exchange token", async ({ setProcessType }) => {
		...
		const { data } = await client.$fetch<any>("/electron/token", {
			...
			onResponse: async (ctx) => {
				...
				expect(cookies.has("reign-auth.session_token")).toBe(true);
			},
		});
	...
	it("authenticate should throw error if called outside browser process", async ({
		...
		).rejects.toThrow(ReignAuthError);
		await expect(
			authenticate({
				$fetch: client.$fetch,
				options,
	...
	it("IPC authenticate bridge should exchange token via invoke", async ({
		...
		const authenticateHandler = mockElectron.ipcMain.handle.mock.calls.find(
			(call) => call[0] === "reign-auth:authenticate",
	...
	describe("transferUser", () => {
		...
		it("should set redirect cookie on success", async () => {
			...
			expect(cookies.has("reign-auth.electron")).toBe(true);
		});
	});
	...
	describe("cookies", () => {
		...
		it("should not trigger infinite refetch with non-./src/cookies", async ({
			...
			const { hasReignAuthCookies } = await import("../src/cookies");
			...
			expect(hasReignAuthCookies(customPrefixHeader, "reign-auth")).toBe(
				false,
			);
			...
			expect(hasReignAuthCookies(securePasskeyHeader, "reign-auth")).toBe(
				true,
			);

			// Custom passkey cookie name should be detected
			const customPasskeyHeader = "reign-auth-custom-challenge=xyz; Path=/";
			expect(hasReignAuthCookies(customPasskeyHeader, "reign-auth")).toBe(
				true,
			);
		});
		...
		it("should allow independent cookiePrefix configuration", async () => {
			...
			expect(hasReignAuthCookies(customCookieHeader, "reign-auth")).toBe(
				false,
			);
		});
		...
		it("should support array of cookie prefixes", async () => {
			...
			expect(
				hasReignAuthCookies(secureHeader, ["reign-auth", "my-app"]),
			).toBe(true);
	...
	it("should register ipc handlers", async ({ setProcessType }) => {
		...
		const prefix = `${(options as any).channelPrefix ?? "reign-auth"}:`;
	...
	it("should not store when encryption fails", async ({ setProcessType }) => {
		...
		expect(client.getCookie()).not.toMatch(
			/reign-auth\.session_token=[a-zA-Z0-9_-]{10,}/,
		);
	});
	...
	it("should use memory storage when encryption is unavailable", async ({
		...
		try {
			...
			expect(memoryFallbackStorage.has("reign-auth.cookie")).toBe(false);
		} finally {
