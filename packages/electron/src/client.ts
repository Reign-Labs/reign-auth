...
import type { ReignAuthClientPlugin, ClientStore } from "reign-auth";
import { isDevelopment, isTest } from "reign-auth";
...
export const electronClient = <O extends ElectronClientOptions>(options: O) => {
	const opts = {
		storagePrefix: "reign-auth",
		cookiePrefix: "reign-auth",
		channelPrefix: "reign-auth",
		...
		fetchPlugins: [
			{
				...
				hooks: {
					onSuccess: async (context) => {
						...
						if (setCookie) {
							if (hasReignAuthCookies(setCookie, opts.cookiePrefix)) {
