import type { ReignAuthClientOptions } from "@reign-auth/core";
...
import type { BetterFetch, CreateFetchOption } from "@reign-auth/fetch";
import { APIError, getBaseURL, safeJSONParse } from "reign-auth";
import { signInSocial } from "reign-auth/api";
...
export const kElectron = Symbol.for("reign-auth:electron");
...
export async function requestAuth(
	clientOptions: ReignAuthClientOptions | undefined,
...
export async function authenticate({
	...
	if (!codeVerifier) {
		throw new ReignAuthError("Code verifier not found.");
	}
