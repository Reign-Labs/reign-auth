...
import { toNodeHandler } from "reign-auth/node";
import type { GenericOAuthConfig } from "reign-auth/plugins/generic-oauth";
import { genericOAuth } from "reign-auth/plugins/generic-oauth";
import { jwt } from "reign-auth/plugins/jwt";
import { multiSession } from "reign-auth/plugins/multi-session";
import type { Organization } from "reign-auth/plugins/organization";
import { organization } from "reign-auth/plugins/organization";
import { getTestInstance } from "reign-auth/test";
import { APIError } from "reign-call";
...
describe("oauth - prompt", async () => {
	...
	it("consent - should sign in", async ({ onTestFinished }) => {
		...
		expect(callbackURL).toContain("/success");
		expect(headers.get("cookie")).toContain("reign-auth.session_token=");
	});
...
describe("oauth - config", () => {
	...
	/**
	 * @see https://github.com/Reign-Labs/reign-auth/issues/8017
	 */
...
describe("oauth - rate limiting", () => {
	...
	// See packages/reign-auth/src/api/rate-limiter/rate-limiter.test.ts
});
