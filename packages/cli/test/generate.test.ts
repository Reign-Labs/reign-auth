...
import { drizzleAdapter } from "reign-auth/adapters/drizzle";
import { prismaAdapter } from "reign-auth/adapters/prisma";
import { organization, twoFactor, username } from "reign-auth/plugins";
...
describe("generate", async () => {
	...
	it("should treat fields with omitted required as notNull (default true)", async () => {
		const pluginWithOmittedRequired = (): ReignAuthPlugin => ({
	...
	const testPlugin = (): ReignAuthPlugin => {
		return {
			id: "test",
			schema: {
				test: {
	...
	const multiRelationPlugin = (): ReignAuthPlugin => {
		return {
			id: "multi-relation",
			schema: {
				project: {
