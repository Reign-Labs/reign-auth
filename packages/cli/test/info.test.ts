...
describe("info command", () => {
	...
	it("should display system information without auth config", async () => {
		...
		// Reign Auth config should have an error since no auth file exists
		expect(output.reignAuth).toHaveProperty("version");
		expect(output.reignAuth.config).toBeNull();
	});
	...
	it("should load and sanitize auth configuration", async () => {
		...
		expect(output.reignAuth.config).toBeDefined();
		expect(output.reignAuth.config.secret).toBe("[REDACTED]");

		// Check social providers are sanitized
		expect(output.reignAuth.config.socialProviders).toBeDefined();
		expect(output.reignAuth.config.socialProviders.github.clientId).toBe(
			"[REDACTED]",
		);
		expect(output.reignAuth.config.socialProviders.github.clientSecret).toBe(
			"[REDACTED]",
		);
		expect(output.reignAuth.config.socialProviders.google.clientId).toBe(
			"[REDACTED]",
		);
		expect(output.reignAuth.config.socialProviders.google.clientSecret).toBe(
			"[REDACTED]",
		);

		// Check non-sensitive data is preserved
		expect(output.reignAuth.config.emailAndPassword).toEqual({
			enabled: true,
		});
		expect(output.reignAuth.config.baseURL).toBe("https://example.com");
	});
	...
	it("should support custom config path", async () => {
		...
		expect(output.reignAuth.config).toBeDefined();
		expect(output.reignAuth.config.appName).toBe("Custom Config App");
		expect(output.reignAuth.config.secret).toBe("[REDACTED]");
		expect(output.reignAuth.config.emailAndPassword).toEqual({
			enabled: true,
		});
	...
	it("should sanitize plugin configurations", async () => {
		...
		await fs.writeFile(
			path.join(tmpDir, "auth.ts"),
			`import { reignAuth } from "reign-auth";
			import { twoFactor, organization } from "reign-auth/plugins";
		...
		expect(output.reignAuth.config.plugins).toBeDefined();
		expect(Array.isArray(output.reignAuth.config.plugins)).toBe(true);

		// Plugin sensitive data should be redacted
		const plugins = output.reignAuth.config.plugins;
