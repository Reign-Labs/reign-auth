...
describe("initAction", () => {
	...
	testWithTmpDir(
		...
		async ({ tmp, skip }) => {
			...
			mockPrompts.mockImplementation(async (questions: any) => {
				...
				// Install Reign Auth
				if (question.message?.includes("install reign-auth")) {
					return { value: true };
				}
			...
			expect(mockInstallDependencies).toHaveBeenCalledWith(
				expect.objectContaining({
					...
					dependencies: expect.arrayContaining(["reign-auth"]),
				}),
			);
	...
	testWithTmpDir("should handle existing auth config file", async ({ tmp }) => {
		...
		const originalContent = "export const auth = reignAuth({});";
		...
		expect(authConfigContent).toContain("reignAuth");
	});
	...
	testWithTmpDir(
		...
		async ({ tmp }) => {
			...
			await fs.writeFile(
				path.join(tmp, ".env"),
				"REIGN_AUTH_SECRET=test\nREIGN_AUTH_URL=http://localhost:3000",
			);
