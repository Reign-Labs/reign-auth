...
describe("installDependencies", () => {
	...
	testWithTmpDir(
		...
		async ({ tmp }) => {
			await installDependencies({
				dependencies: ["reign-auth", "drizzle-orm"],
				packageManager: "npm",
				cwd: tmp,
			});

			expect(mockExec).toHaveBeenCalledWith(
				"npm install --force reign-auth drizzle-orm",
				{ cwd: tmp },
				expect.any(Function),
			);
		},
	...
	testWithTmpDir(
		...
		async ({ tmp }) => {
			...
			expect(mockExec).toHaveBeenCalledWith(
				"npm install --force --save-dev reign-auth",
				{ cwd: tmp },
				expect.any(Function),
			);
		},
	...
	testWithTmpDir(
		...
		async ({ tmp }) => {
			...
			expect(mockExec).toHaveBeenCalledWith(
				"npm install --force --save-optional reign-auth",
				{ cwd: tmp },
				expect.any(Function),
			);
		},
