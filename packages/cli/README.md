# Reign Auth CLI

Reign Auth comes with a built-in CLI to help you manage the database schema
needed for both core functionality and plugins.

### **Init**

The CLI includes an `init` command to add Reign Auth to your project.

```bash title="terminal"
npx auth@latest init
```

### **Generate**

The `generate` command creates the schema required by Reign Auth.
If you’re using a database adapter like Prisma or Drizzle, this command will
generate the right schema for your ORM.
If you’re using the built-in Kysely adapter, it will generate an SQL file you
can run directly on your database.

```bash title="terminal"
npx auth@latest generate
```

### **Migrate**

The `migrate` command applies the Reign Auth schema directly to your database.
This is available if you’re using the built-in Kysely adapter.
For other adapters, you’ll need to apply the schema using your ORM’s migration
tool.

```bash title="terminal"
npx auth@latest migrate
```

### **Secret**

The CLI also provides a way to generate a secret key for your Reign Auth
instance.

```bash title="terminal"
npx auth@latest secret
```

## License

MIT
