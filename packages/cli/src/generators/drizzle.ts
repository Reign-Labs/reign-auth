...
import { initGetFieldName, initGetModelName } from "reign-auth/adapters";
import type { ReignAuthDBSchema, DBFieldAttribute } from "reign-auth/db";
import { getAuthTables } from "reign-auth/db";
import type { ReignAuthOptions } from "reign-auth/types";
...
export const generateDrizzleSchema: SchemaGenerator = async ({
	...
	if (!databaseType) {
		throw new Error(
			`Database provider type is undefined during Drizzle schema generation. Please define a \`provider\` in the Drizzle adapter config. Read more at https://auth.reign-labs.com/docs/adapters/drizzle`,
		);
	}
