...
function getPathAliases(cwd: string): Record<string, string> | null {
	...
	try {
		...
		console.error(error);
		throw new ReignAuthError("Error parsing tsconfig.json");
	}
}
/**
...
export async function getConfig({
	...
	try {
		let configFile: ReignAuthOptions | null = null;
		if (configPath) {
			...
			if (!("auth" in config) && !isDefaultExport(config)) {
				...
				console.error(
					`[#reign-auth]: Couldn't read your auth config in ${resolvedPath}. Make sure to default export your auth instance or to export as a variable named auth.`,
				);
				process.exit(1);
			}
				...
				try {
					...
					if (hasConfig) {
						...
						if (!configFile) {
							...
							console.error("[#reign-auth]: Couldn't read your auth config.");
							console.log("");
							console.log(
								"[#reign-auth]: Make sure to default export your auth instance or to export as a variable named auth.",
							);
							process.exit(1);
						}
						break;
					...
					console.error("[#reign-auth]: Couldn't read your auth config.", e);
					process.exit(1);
				}
			}
		}
