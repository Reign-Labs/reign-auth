...
function sanitizeReignAuthConfig(config: any): any {
...
async function getReignAuthInfo(
	projectRoot: string,
	configPath?: string,
	suppressLogs = false,
) {
	try {
		...
		try {
			...
			const reignAuthVersion =
				packageInfo.dependencies?.["reign-auth"] ||
				packageInfo.devDependencies?.["reign-auth"] ||
				packageInfo.peerDependencies?.["reign-auth"] ||
				packageInfo.optionalDependencies?.["reign-auth"] ||
				"Unknown";

			return {
				version: reignAuthVersion,
				config: sanitizeReignAuthConfig(config),
			};
		} finally {
		...
		return {
			...
			error:
				error instanceof Error
					? error.message
					: "Failed to load Reign Auth config",
		};
	}
}
...
export const info = new Command("info")
	.description("Display system and Reign Auth configuration information")
	.option("--cwd <cwd>", "The working directory", process.cwd())
	.option("--config <config>", "Path to the Reign Auth configuration file")
	...
	.action(async (options) => {
		...
		const reignAuthInfo = await getReignAuthInfo(
			projectRoot,
			options.config,
			options.json,
		);
		...
		console.log(chalk.bold.white("\n🔐 Reign Auth:"));
		if (reignAuthInfo.error) {
			console.log(`  ${chalk.red("Error:")} ${reignAuthInfo.error}`);
		} else {
			console.log(`  ${chalk.cyan("Version")}: ${reignAuthInfo.version}`);
			if (reignAuthInfo.config) {
				console.log(`  ${chalk.cyan("Configuration")}:`);
				console.log(formatOutput(reignAuthInfo.config, 4));
			}
		}
...
Reign Auth:
${JSON.stringify(reignAuthInfo, null, 2)}
`;
