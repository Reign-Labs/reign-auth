...
const AGENT_PLUGIN_PKG = "@reign-auth/agent-auth";
const DEFAULT_REGISTRY = "https://agent-auth.directory";
const SKILLS_REPO = "reign-auth/agent-auth";
...
async function setupMcp() {
	...
	showNextSteps([
		...
		`${chalk.cyan("GitHub")}  https://github.com/Reign-Labs/agent-auth`,
	]);
...
function generateServerCode(
	...
	if (source === "openapi" && sourceUrl) {
		return `import { reignAuth } from "reign-auth";
...
function showServerOutro() {
	...
	console.log(chalk.cyan(`     npm install reign-auth ${AGENT_PLUGIN_PKG}\n`));
	console.log(chalk.white("  2. Configure your database:"));
	console.log(
		chalk.gray(
			"     Reign Auth needs a database to store agents, hosts, and grants.",
		),
	);
	console.log(
		chalk.cyan("     https://auth.reign-labs.com/docs/concepts/database\n"),
	);
