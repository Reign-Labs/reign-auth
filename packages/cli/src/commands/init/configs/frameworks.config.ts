export const FRAMEWORKS = [
	{
		...
		dependency: "astro",
		authClient: {
			importPath: "reign-auth/react", // assume react is used for astro
		},
		routeHandler: {
...
export const ALL: APIRoute = async (ctx) => {
	...
	// todo: remove in future versions
	{
		...
		authClient: {
			importPath: "reign-auth/react",
		},
		routeHandler: {
			path: "app/lib/auth.server.ts",
			code: `import { reignAuth } from "reign-auth"
...
export default defineEventHandler((event) => {
	...
	},
	{
		...
		authClient: {
			importPath: "reign-auth/svelte",
		},
		routeHandler: {
			path: `hooks.server.ts`,
			code: `import { auth } from "$lib/auth";
import { svelteKitHandler } from "reign-auth/svelte-kit";
...
export async function handle({ event, resolve }) {
  return svelteKitHandler({ event, resolve, auth, building });
	...
	},
	{
		...
		authClient: {
			importPath: "reign-auth/solid",
		},
		routeHandler: {
			path: `routes/api/auth/*auth.ts`,
			code: `import { auth } from "~/lib/auth";
import { toSolidStartHandler } from "reign-auth/solid-start";
...
export const { GET, POST } = toSolidStartHandler(auth);`,
	...
	},
	{
		...
		authClient: {
			importPath: "reign-auth/react", // assume react is used for tanstack start
		},
		routeHandler: {
