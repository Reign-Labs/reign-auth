...
/**
 * Create a minimal ReignAuthOptions config for schema generation
 */
...
export async function initAction(opts: any) {
	...
	console.log(
		...
		"\n" +
			[
				`   ██  ████`,
				`   ████  ██  ${chalk.bold(`Reign Auth CLI`)} ${chalk.dim(`(${cliVersion})`)}`,
				`   ██  ████  ${chalk.gray("Welcome to the Reign Auth CLI! Let's get you set up.")}`,
			]
				// .map((x) => x.padStart(10))
				.join("\n"),
		// 	{
	...
	// Install Reign Auth
	await (async () => {
		const hasReignAuth = await hasDependency(packageJson, "reign-auth");
		if (hasReignAuth) return;
		await nextStep("Install Reign Auth");

		const shouldInstallReignAuth = await confirm({
			message: `Would you like to install reign-auth using ${chalk.bold(pm)}?`,
			initial: true,
		});
		if (isCancel(shouldInstallReignAuth)) {
			cancel("✋ Operation cancelled.");
			process.exit(0);
		}

		if (shouldInstallReignAuth) {
			depsToInstall.set("reign-auth", {
				prod: true,
			});
		}
	})();
		...
		if (envFiles.size === 0) {
			...
			if (shouldCreateEnv) {
				const { providedSecret } = await prompts({
					type: "text",
					name: "providedSecret",
					message: `Reign Auth secret (used for encryption, hashing, and signing). ${chalk.dim("(Press Enter to auto generate)")}`,
				});
				...
				const { providedURL } = await prompts({
					type: "text",
					name: "providedURL",
					message: `Reign Auth Base URL (your auth server URL):`,
				...
				const envs = [
					`REIGN_AUTH_SECRET="${secret}"`,
					`REIGN_AUTH_URL="${providedURL}"`,
				];
			...
			if (confirmed) {
				...
				for (const v of missingVars) {
					if (v === "REIGN_AUTH_SECRET") {
						...
						envs.push(
							`REIGN_AUTH_SECRET="${providedSecret || generateSecretHash()}"`,
						);
					} else if (v === "REIGN_AUTH_URL") {
						const { providedURL } = await prompts({
							type: "text",
							name: "providedURL",
							message: `Reign Auth base URL (your auth server URL):`,
						...
						envs.push(`REIGN_AUTH_URL="${providedURL}"`);
					}
				}
		...
		if (filesToUpdate) {
			...
			for (const file of filesToUpdate) {
				const envs = missingEnvVars
					...
					.var.map((v) => {
						if (v === "REIGN_AUTH_SECRET") {
							return `REIGN_AUTH_SECRET="${secretHash}"`;
						}
						if (v === "REIGN_AUTH_URL") {
							return 'REIGN_AUTH_URL="http://localhost:3000"';
						}
						return `${v}=${v}`;
					});
				filesToWrite.push(() => updateEnvFiles([file], envs));
	...
	if (!hasAuthConfigAlready) {
		await nextStep("Create A Reign Auth Instance");
		...
		const boilerplateCode = `import { reignAuth } from "reign-auth";
...
`;
	...
	const connectResponse = await prompts({
		...
		name: "connect",
		message:
			"Would you like to connect your app to Reign Auth infrastructure?",
		initial: true,
	});
	...
	if (connectResponse.connect === undefined) {
		console.log(
			chalk.yellow("\n✖ ") +
				"Setup cancelled before connecting to Reign Auth infrastructure.\n",
		);
		return;
	}
	// If the user cancels the prompt, `connect` will be undefined.
	...
	if (connectResponse.connect) {
		await open("https://dash.auth.reign-labs.com/onboarding");
		console.log(
			chalk.cyan("\n→ ") +
				"Opening Reign Auth onboarding in your browser...\n",
		);
	}
