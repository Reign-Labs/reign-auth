	...
	describe("auth client import paths", () => {
		testWithTmpDir(
			...
			async ({ tmp }) => {
				...
				expect(result?.authClient?.importPath).toBe("reign-auth/react");
			},
		);
		...
		testWithTmpDir(
			...
			async ({ tmp }) => {
				...
				expect(result?.authClient).not.toBeNull();
				expect(result?.authClient?.importPath).toBe("reign-auth/vue");
			},
		);
		...
		testWithTmpDir(
			...
			async ({ tmp }) => {
				...
				expect(result?.authClient).not.toBeNull();
				expect(result?.authClient?.importPath).toBe("reign-auth/svelte");
			},
		);
		...
		testWithTmpDir(
			...
			async ({ tmp }) => {
				...
				expect(result?.authClient).not.toBeNull();
				expect(result?.authClient?.importPath).toBe("reign-auth/solid");
			},
		);
