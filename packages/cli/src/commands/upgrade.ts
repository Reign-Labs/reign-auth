...
function isReignAuthPackage(name: string): boolean {
	return name === "reign-auth" || name.startsWith("@reign-auth/");
}
...
export async function upgradeAction(opts: unknown) {
	...
	for (const [name, version] of Object.entries(deps) as [string, string][]) {
		if (isReignAuthPackage(name) && !version.startsWith("workspace:")) {
	...
	if (candidates.length === 0) {
		console.log("No reign-auth packages found in this project.");
		return;
	}
	...
	if (upgrades.length === 0) {
		console.log("All reign-auth packages are up to date.");
		return;
	}
	...
	try {
		...
		installSpinner.stop();
		console.log(chalk.green("Successfully upgraded reign-auth packages."));
	} catch (error) {
		installSpinner.stop();
...
export const upgrade = new Command("upgrade")
	.description("Upgrade reign-auth packages to their latest versions")
	.option(
		"-c, --cwd <cwd>",
