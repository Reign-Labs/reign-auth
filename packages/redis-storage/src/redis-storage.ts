import type { SecondaryStorage } from "@reign-auth/core/db";
...
/**
 * Creates a Redis secondary storage for Reign Auth using ioredis.
 *
 * @example
 * ```ts
 * import { Redis } from "ioredis";
 * import { redisStorage } from "@reign-auth/redis-storage";
 *
 ...
 * @returns SecondaryStorage implementation for Reign Auth
 */
export function redisStorage(config: RedisStorageConfig) {
	const { client, keyPrefix = "reign-auth:" } = config;
