...
import { createAdapterFactory } from "@reign-auth/core/db/adapter";
...
export const drizzleAdapter = (db: DB, config: DrizzleAdapterConfig) => {
	let lazyOptions: ReignAuthOptions | null = null;
	const createCustomAdapter =
		...
		({ getFieldName, getDefaultFieldName, options }) => {
			 ...
			 *
			 * When `usePlural` is false (default), Reign Auth uses singular model
			...
			return {
				...
				async deleteMany({ model, where }) {
					...
					if (typeof count !== "number") {
						logger.error(
							"[Drizzle Adapter] The result of the deleteMany operation is not a number. This is likely a bug in the adapter. Please report this issue to the Reign Auth team.",
							{ res, model, where },
						);
					}
					return count;
	...
	adapterOptions = {
		config: {
			...
			customTransformOutput: ({ data, fieldAttributes }) => {
				...
				// one such example case is https://github.com/Reign-Labs/reign-auth/issues/7819
	...
	return (options: ReignAuthOptions): DBAdapter<ReignAuthOptions> => {
		lazyOptions = options;
