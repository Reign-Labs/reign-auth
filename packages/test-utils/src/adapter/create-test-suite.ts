...
} from "@reign-auth/core/db/adapter";
import { TTY_COLORS } from "@reign-auth/core/env";
...
export type TestEntry =
	...
	| {
			migrateReignAuth?: ReignAuthOptions;
			test: (context: {
				readonly skip: {
					(note?: string | undefined): never;
					(condition: boolean, note?: string | undefined): void;
...
/**
 * Deep equality comparison for ReignAuthOptions.
...
>(
	suiteName: string,
	config: {
		defaultReignAuthOptions?: ReignAuthOptions | undefined;
		/**
		 * Helpful if the default Reign Auth options require migrations to be run.
		 */
	...
	tests: (
		helpers: {
			...
			modifyReignAuthOptions: (
				options: ReignAuthOptions,
				shouldRunMigrations: boolean,
			) => Promise<ReignAuthOptions>;
			getReignAuthOptions: () => ReignAuthOptions;
			sortModels: (
				models: Array<
					Record<string, any> & {
						id: string;
			 ...
			 * data and it will return the correct reign-auth-expected transformed data.
			 *
			 * Eg:
	...
	return (
		...
		return async (helpers: {
			adapter: () => Promise<DBAdapter<ReignAuthOptions>>;
			log: Logger;
			...
			const wrapperAdapter = (
				overrideOptions?: ReignAuthOptions | undefined,
			) => {
				const options = deepmerge(
					deepmerge(
						helpers.getReignAuthOptions(),
						config?.defaultReignAuthOptions || {},
					),
					overrideOptions || {},
				);
				const adapterConfig = {
			...
			const cleanupCreatedRows = async () => {
				...
				for (const model of Object.keys(createdRows)) {
					for (const row of createdRows[model]!) {
						const schema = getAuthTables(helpers.getReignAuthOptions());
			...
			// Track current applied ReignAuth options state
			let currentAppliedOptions: ReignAuthOptions | null = null;
			...
			/**
			 * Apply ReignAuth options and run migrations if needed.
			 * Tracks migration statistics.
			 */
			const applyOptionsAndMigrate = async (
				options: ReignAuthOptions | Partial<ReignAuthOptions>,
				...
				if (optionsChanged || forceMigrate) {
					adapter = await helpers.adapter();
					await helpers.modifyReignAuthOptions(finalOptions);
			...
			const modifyReignAuthOptions = async (
				opts: ReignAuthOptions,
				shouldRunMigrations: boolean,
			) => {
			...
			const fullTests = tests(
				{
					...
					getAuth: async () => {
						...
						const auth = reignAuth({
							...helpers.getReignAuthOptions(),
							...(config?.defaultReignAuthOptions || {}),
							database: (options: ReignAuthOptions) => {
					...
					insertRandom,
					modifyReignAuthOptions,
					getReignAuthOptions: helpers.getReignAuthOptions,
					sortModels,
					tryCatch,
			...
			test(`\n${TTY_COLORS.fg.white}${" ".repeat(3)}${dash.repeat(35)} [${TTY_COLORS.fg.magenta}${suiteName}${TTY_COLORS.fg.white}] ${dash.repeat(35)}`, async () => {
				...
				if (config.defaultReignAuthOptions && !allDisabled) {
			...
			const extractTestEntry = (
				...
				return {
					testFn: entry.test,
					migrateReignAuth: entry.migrateReignAuth,
				};
			};
			...
			const testEntries = Object.entries(fullTests).map(([name, entry]) => {
				const { testFn, migrateReignAuth } = extractTestEntry(
					entry as TestEntry,
				);
				return { name, testFn, migrateReignAuth };
			});

			/**
			 * Group tests by their migrateReignAuth options.
			 * Tests with equal migration options are grouped together.
			 */
			type TestGroup = {
				migrationOptions: ReignAuthOptions | null | undefined;
				testIndices: number[];
			};
			...
			const groupTestsByMigrationOptions = (): TestGroup[] => {
				...
				for (let i = 0; i < testEntries.length; i++) {
					const { migrateReignAuth } = testEntries[i]!;
					const isSkipped =
						(allDisabled &&
					...
					if (isSkipped) {
						...
						groups.push({
							migrationOptions: migrateReignAuth,
							testIndices: [i],
						});
						continue;
					}
					...
					if (
						currentGroup &&
						deepEqual(currentGroup.migrationOptions, migrateReignAuth)
					) {
			...
			let currentGroupMigrationOptions: ReignAuthOptions | null | undefined =
				null;

			for (let i = 0; i < testEntries.length; i++) {
				const { name: testName, testFn, migrateReignAuth } = testEntries[i]!;
					...
					async ({ onTestFailed, skip }) => {
						...
						await (async () => {
							...
							// If this test is not in a group or not first in group, check if migration is needed
								...
								!deepEqual(currentGroupMigrationOptions, migrateReignAuth)
							) {
								await applyOptionsAndMigrate(thisMigration, true);
								currentGroupMigrationOptions = migrateReignAuth;
							}
						})();
