import type { Awaitable, ReignAuthOptions } from "@reign-auth/core";
import type { DBAdapter } from "@reign-auth/core/db/adapter";
import { deepmerge, initGetModelName } from "@reign-auth/core/db/adapter";
...
export const testAdapter = async ({
	...
	) => Awaitable<(options: ReignAuthOptions) => DBAdapter<ReignAuthOptions>>;
	/**
	 * A function that will run the database migrations.
	 */
	runMigrations: (reignAuthOptions: ReignAuthOptions) => Promise<void> | void;
	/**
	 * Any potential reign-auth options overrides.
	 */
	overrideReignAuthOptions?: (
		reignAuthOptions: ReignAuthOptions,
	) => ReignAuthOptions;
	/**
	...
	const defaultBAOptions = {} satisfies ReignAuthOptions;
	let reignAuthOptions = (() => {
		return {
			...defaultBAOptions,
			...(overrideReignAuthOptions?.(defaultBAOptions) || {}),
		} satisfies ReignAuthOptions;
	})();

	let adapter: DBAdapter<ReignAuthOptions> = (
		await getAdapter(reignAuthOptions)
	)(reignAuthOptions);
	...
	const refreshAdapter = async (reignAuthOptions: ReignAuthOptions) => {
		adapter = (await getAdapter(reignAuthOptions))(reignAuthOptions);
	};
	...
	const cleanup = async () => {
		...
		await refreshAdapter(reignAuthOptions);
		const getAllModels = getAuthTables(reignAuthOptions);
	...
	const migrate = async () => {
		...
		try {
			await runMigrations(reignAuthOptions);
		} catch (error) {
			...
			describe(adapterDisplayName, async () => {
				...
				for (const testSuite of tests) {
					await testSuite({
						...
						modifyReignAuthOptions: async (options) => {
							const newOptions = deepmerge(defaultBAOptions, options);
							reignAuthOptions = deepmerge(
								newOptions,
								overrideReignAuthOptions?.(newOptions) || {},
							);
							await refreshAdapter(reignAuthOptions);
							return reignAuthOptions;
						},
						cleanup,
						prefixTests,
