...
import { safeJSONParse } from "@reign-auth/core/utils/json";
import {
	parseSetCookieHeader,
	SECURE_COOKIE_PREFIX,
	stripSecureCookiePrefix,
} from "reign-auth/cookies";
...
interface ExpoClientOptions {
	...
	/**
	 * Prefix(es) for server cookie names to filter (e.g., "reign-auth.session_token")
	 * This is used to identify which cookies belong to reign-auth to prevent
...
/**
 * Check if the Set-Cookie header contains reign-auth cookies.
 * This prevents infinite refetching when non-reign-auth cookies (like third-party cookies) change.
 *
 * Supports multiple cookie naming patterns:
 * - Default: "reign-auth.session_token", "reign-auth-passkey", "__Secure-reign-auth.session_token"
 ...
 * - Multiple prefixes: ["reign-auth", "my-app"] matches cookies starting with any of the prefixes
 *
 * @param setCookieHeader - The Set-Cookie header value
 * @param cookiePrefix - The cookie prefix(es) to check for. Can be a string, array of strings, or empty string.
 * @returns true if the header contains reign-auth cookies, false otherwise
 */
export function hasReignAuthCookies(
	...
	// Check if any cookie is a reign-auth cookie
	for (const name of cookies.keys()) {
		...
		for (const prefix of prefixes) {
			if (prefix) {
				...
				// This matches all reign-auth cookies including session cookies, passkey cookies, etc.
				if (nameWithoutSecure.startsWith(prefix)) {
					return true;
				}
			} else {
				// When prefix is empty, check for common reign-auth cookie patterns
 ...
 *
 * @see https://github.com/Reign-Labs/reign-auth/issues/5426
 *
...
export const expoClient = (opts: ExpoClientOptions) => {
	...
	const storagePrefix = opts?.storagePrefix || "reign-auth";
	...
	const cookiePrefix = opts?.cookiePrefix || "reign-auth";
		...
		fetchPlugins: [
			{
				...
				hooks: {
					async onSuccess(context) {
						...
						if (setCookie) {
							// Only process and notify if the Set-Cookie header contains reign-auth cookies
							// This prevents infinite refetching when other cookies (like Cloudflare's __cf_bm) are present
							if (hasReignAuthCookies(setCookie, cookiePrefix)) {
...
export { parseSetCookieHeader } from "reign-auth/cookies";
