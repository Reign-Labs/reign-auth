import type { CookieOptions, EndpointContext } from "reign-call";
import type {
	Account,
	ReignAuthDBSchema,
	ModelNames,
...
import type { ReignAuthCookie, ReignAuthCookies } from "./cookie";
 ...
 * @internal
 */
type InferPluginID<O extends ReignAuthOptions> =
...
type InferPluginOptions<
	O extends ReignAuthOptions,
	ID extends ReignAuthPluginRegistryIdentifier | LiteralString,
> =
	O["plugins"] extends Array<infer P>
		? P extends ReignAuthPlugin
			? P["id"] extends ID
...
export interface ReignAuthPluginRegistry<AuthOptions, Options> {}
export type ReignAuthPluginRegistryIdentifier = keyof ReignAuthPluginRegistry<
	unknown,
	unknown
>;

export type GenericEndpointContext<
	Options extends ReignAuthOptions = ReignAuthOptions,
> = EndpointContext<string, any, any, any, any, any, any, AuthContext<Options>>;

export interface InternalAdapter<
	_Options extends ReignAuthOptions = ReignAuthOptions,
> {
	createOAuthUser(
...
type CheckPasswordFn<Options extends ReignAuthOptions = ReignAuthOptions> = (
	userId: string,
	ctx: GenericEndpointContext<Options>,
) => Promise<boolean>;

export type PluginContext<Options extends ReignAuthOptions> = {
	getPlugin: <
		ID extends ReignAuthPluginRegistryIdentifier | LiteralString,
	...
	>(
		...
		| (ID extends ReignAuthPluginRegistryIdentifier
				? ReignAuthPluginRegistry<Options, PluginOptions>[ID] extends {
						creator: infer C;
					}
	 ...
	 * ```
	 */
	hasPlugin: <ID extends ReignAuthPluginRegistryIdentifier | LiteralString>(
		pluginId: ID,
...
export type AuthContext<Options extends ReignAuthOptions = ReignAuthOptions> =
