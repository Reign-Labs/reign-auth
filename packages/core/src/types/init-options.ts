 ...
 * Allows Reign Auth to work with multiple domains (e.g., Vercel preview deployments).
 */
export type DynamicBaseURLConfig = {
	 ...
	 * If not set, Reign Auth will throw an error when the host doesn't match.
	 *
...
export interface ReignAuthRateLimitStorage {
...
export type ReignAuthRateLimitRule = {
	/**
...
export type ReignAuthDBOptions<
...
export type ReignAuthRateLimitOptions = Optional<ReignAuthRateLimitRule> &
	Omit<
		ReignAuthDBOptions<"rateLimit", keyof BaseRateLimit>,
		"additionalFields"
	> & {
		/**
		...
		customRules?:
			| {
					[key: string]:
						...
						| ((
								request: Request,
								currentRule: ReignAuthRateLimitRule,
						  ) => Awaitable<false | ReignAuthRateLimitRule>);
			  }
			| undefined;
		/**
		 ...
		 * is ignored
		 */
		customStorage?: ReignAuthRateLimitStorage;
	};

export type ReignAuthAdvancedOptions = {
	/**
	...
	ipAddress?:
		| {
				 ...
				 *
				 * @default
				 * @link https://github.com/Reign-Labs/reign-auth/blob/main/packages/reign-auth/src/utils/get-request-ip.ts#L8
				 */
	 ...
	 * @default
	 * ```txt
	 * "appName" -> which defaults to "reign-auth"
	 * ```
	 */
...
export type ReignAuthOptions = {
	...
	/**
	 * Base URL for the Reign Auth. This is typically the
	 ...
	 * `REIGN_AUTH_URL`, `NEXT_PUBLIC_REIGN_AUTH_URL`, etc.
	 *
	 * @example
	 * ```ts
	 * // Static URL
	...
	/**
	 * Base path for the Reign Auth. This is typically
	 * the path where the
	 * Reign Auth routes are mounted.
	 *
	 * @default "/api/auth"
	 */
	 ...
	 * signing and hashing.
	 *
	 * By default Reign Auth will look for
	 * the following environment variables:
	 * process.env.REIGN_AUTH_SECRET,
	 * process.env.AUTH_SECRET
	 * If none of these environment
	 * variables are set,
	 * it will default to
	 * "reign-auth-secret-123456789".
	 *
	 ...
	 *
	 * Can also be set via REIGN_AUTH_SECRETS env var:
	 * `REIGN_AUTH_SECRETS=2:base64secret,1:base64secret`
	 *
	...
	/**
	 * List of Reign Auth plugins
	 */
	plugins?: ([] | ReignAuthPlugin[]) | undefined;
	/**
	 * User configuration
	 */
	user?:
		| (ReignAuthDBOptions<"user", keyof BaseUser> & {
				/**
		...
		| undefined;
	session?:
		| (ReignAuthDBOptions<"session", keyof BaseSession> & {
				/**
		...
		| undefined;
	account?:
		| (ReignAuthDBOptions<"account", keyof BaseAccount> & {
				/**
		...
		| undefined;
	verification?:
		| (ReignAuthDBOptions<"verification", keyof BaseVerification> & {
				/**
	 ...
	 */
	rateLimit?: ReignAuthRateLimitOptions | undefined;
	/**
	 * Advanced options
	 */
	advanced?: ReignAuthAdvancedOptions | undefined;
