...
import { APIError, ReignAuthError } from "../error";
...
export const cognito = (options: CognitoOptions) => {
	if (!options.domain || !options.region || !options.userPoolId) {
		...
		throw new ReignAuthError("DOMAIN_AND_REGION_REQUIRED");
	}
	...
	return {
		...
		async createAuthorizationURL({ state, scopes, codeVerifier, redirectURI }) {
			if (!options.clientId) {
				...
				throw new ReignAuthError("CLIENT_ID_AND_SECRET_REQUIRED");
			}
			...
			if (options.requireClientSecret && !options.clientSecret) {
				...
				throw new ReignAuthError("CLIENT_SECRET_REQUIRED");
			}
