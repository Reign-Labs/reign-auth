...
const createAsIsTransaction =
	<Options extends ReignAuthOptions>(adapter: DBAdapter<Options>) =>
	<R>(fn: (trx: DBTransactionAdapter<Options>) => Promise<R>) =>
		fn(adapter);

export type AdapterFactory<Options extends ReignAuthOptions> = (
	options: Options,
) => DBAdapter<Options>;

export const createAdapterFactory =
	<Options extends ReignAuthOptions>({
			...
			return where.map((w) => {
				...
				if (operator === "in") {
					if (!Array.isArray(value)) {
						throw new ReignAuthError("Value must be an array");
					}
				}
