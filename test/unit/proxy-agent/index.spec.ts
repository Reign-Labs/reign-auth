/**
 * Unit Test: Verify undici ProxyAgent works with reign-auth OAuth flows
 *
 * This test demonstrates that when you set a global dispatcher with ProxyAgent,
 * all fetch() calls made by reign-auth during OAuth flows are routed through it.
 ...
 * Related issue: https://github.com/Reign-Labs/reign-auth/issues/7396
 */
import type { GoogleProfile } from "@reign-auth/core/social-providers";
import { signJWT } from "reign-auth/crypto";
import { getTestInstance } from "reign-auth/test";
import type { Dispatcher } from "undici";
import { getGlobalDispatcher, MockAgent, setGlobalDispatcher } from "undici";
import { afterEach, beforeEach, describe, expect, it } from "vitest";

const DEFAULT_SECRET = "reign-auth-secret-123456789";

describe("ProxyAgent with reign-auth OAuth", () => {
	...
	it("should route GitHub OAuth requests through global dispatcher", async () => {
		...
		// Create reign-auth instance with GitHub OAuth
		...
		// Verify that reign-auth made requests through our global dispatcher (proxy)
	...
	it("should route Google OAuth requests through global dispatcher", async () => {
		...
		// Create reign-auth instance with Google OAuth
		const { client } = await getTestInstance({
			secret: DEFAULT_SECRET,
			socialProviders: {
				google: {
