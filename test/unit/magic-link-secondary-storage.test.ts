...
import { magicLink } from "reign-auth/plugins";
...
/**
 * @see https://github.com/Reign-Labs/reign-auth/issues/8228
 */
describe("magic link with secondary storage (string return)", async () => {
	...
	it("should send and verify magic link", async () => {
		...
		const reignAuthCookie = headers.get("set-cookie");
		expect(reignAuthCookie).toBeDefined();
	});
