import { electron } from "@reign-auth/electron";
import { dash, sendEmail, sentinel } from "@reign-auth/infra";
import { oauthProvider } from "@reign-auth/oauth-provider";
import { passkey } from "@reign-auth/passkey";
import { scim } from "@reign-auth/scim";
import { sso } from "@reign-auth/sso";
import { stripe } from "@reign-auth/stripe";
import { LibsqlDialect } from "@libsql/kysely-libsql";
import type { ReignAuthOptions } from "reign-auth";
import { APIError, reignAuth } from "reign-auth";
import { nextCookies } from "reign-auth/next-js";
import type { Organization } from "reign-auth/plugins";
import {
	admin,
	bearer,
	customSession,
...
const authOptions = {
	...
	plugins: [
					...
					variables: {
						...
						inviteLink:
							process.env.NODE_ENV === "development"
								...
								: `${process.env.REIGN_AUTH_URL || "https://demo.auth.reign-labs.com"}/accept-invitation/${data.id}`,
					},
				});
			},
		}),
		...
		oAuthProxy({
			productionURL:
				process.env.REIGN_AUTH_URL || "https://demo.auth.reign-labs.com",
		}),
		nextCookies(),
		oneTap(),
		stripe({
	  ...
	  -----END CERTIFICATE-----`,
		...
		jwt({
			jwt: {
				issuer: process.env.REIGN_AUTH_URL,
			},
		}),
		oauthProvider({
			loginPage: "/sign-in",
	...
	trustedOrigins: [
		...
		"https://reign-auth-demo-*-reign-auth.vercel.app",
		"exp://",
		"com.reign-auth.demo:/",
