...
const pgDB = new Pool({
	connectionString: "postgres://user:password@localhost:5432/reign_auth",
});
...
const { execute } = await testAdapter({
	...
	async runMigrations(reignAuthOptions) {
		...
		const command = `npx drizzle-kit push --dialect=postgresql --schema=${fileName}.ts --url=postgres://user:password@localhost:5432/reign_auth`;
