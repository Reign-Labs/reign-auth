...
// Create reign_auth database if it doesn't exist
// We need to connect to 'master' database first since 'reign_auth' may not exist yet
const ensureDatabaseExists = async () => {
	try {
		// console.log("Ensuring reign_auth database exists...");
		// Create a temporary connection to 'master' database to create 'reign_auth'
		...
		await masterDB.getExecutor().executeQuery({
			sql: `
				IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = 'reign_auth')
				BEGIN
					CREATE DATABASE reign_auth;
					PRINT 'Database reign_auth created successfully';
				END
				ELSE
				BEGIN
					PRINT 'Database reign_auth already exists';
				END
			`,
			parameters: [],
...
// Create dialect for reign_auth database (after ensuring it exists)
const dialect = new MssqlDialect({
	...
	tedious: {
		...Tedious,
		connectionFactory: createConnectionFactory("reign_auth"),
		TYPES: {
			...Tedious.TYPES,
			DateTime: Tedious.TYPES.DateTime2,
		},
...
const query = async (sql: string, timeoutMs: number = 30000) => {
	...
	try {
		// Ensure we're using the reign_auth database for queries
