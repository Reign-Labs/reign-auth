...
describe("SAML SSO", () => {
	/**
	 * @see https://github.com/Reign-Labs/reign-auth/issues/8695
	 *
	 ...
	 * This test verifies that @reign-auth/sso loads without error under ESM
	 * by using a native ESM import (not createRequire).
	 */
	test("should load @reign-auth/sso via ESM without samlify import error", async () => {
		const mod = await import("@reign-auth/sso");
