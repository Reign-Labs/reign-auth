...
const cliCommands = [
	{ name: "npm", command: "npx reign-auth init" },
	{ name: "yarn", command: "yarn dlx reign-auth init" },
	{ name: "pnpm", command: "pnpm dlx reign-auth init" },
	{ name: "bun", command: "bunx reign-auth init" },
];

const mcpCommands = [
	{ name: "Cursor", command: "npx reign-auth mcp --cursor" },
	{ name: "Claude Code", command: "npx reign-auth mcp --claude-code" },
	{ name: "Open Code", command: "npx reign-auth mcp --open-code" },
	{ name: "Manual", command: "npx reign-auth mcp --manual" },
];

const aiPromptText = `Set up authentication in my project using Reign Auth (reign-auth npm package).

1. Install reign-auth. If I already have a database configured in this project, use that — don't set up a new one.

2. Create lib/auth.ts — call reignAuth() with:
...
3. Create lib/auth-client.ts — use the correct framework import:
   - React/Next.js: "reign-auth/react"
   - Vue: "reign-auth/vue"
   - Svelte: "reign-auth/svelte"
   - Vanilla: "reign-auth/client"

4. Add the API route handler for my framework (e.g. app/api/auth/[...all]/route.ts for Next.js App Router).

5. Add REIGN_AUTH_SECRET to my .env if it doesn't exist (generate a 32+ char secret).

6. Run npx reign-auth migrate to apply database migrations.

Refer to auth.reign-labs.com/docs for exact API and plugin syntax.`;
							...
							{mode === "cli" || mode === "skills" ? (
								<div className="flex items-center justify-between bg-neutral-100/50 dark:bg-[#050505] px-4 py-3">
									...
									<div className="relative">
										{mode === "skills" ? (
											<button
												onClick={() =>
													copy("npx skills add reign-auth/skills")
												}
								...
								<div className="bg-neutral-100/50 dark:bg-[#050505] px-5 py-4">
									<p className="text-[13px] font-medium text-neutral-700 dark:text-neutral-200 leading-relaxed">
										Set up authentication in my project using Reign Auth.
									</p>
									<div className="relative mt-1.5">
										<p className="text-[11px] text-neutral-400 dark:text-neutral-500 leading-relaxed line-clamp-2">
											Install reign-auth. If I already have a database
			...
			<div className="relative">
				...
				<div className="flex items-center justify-center gap-3 mt-5">
					{stats.npmDownloads > 0 && (
						<a
							href="https://www.npmjs.com/package/reign-auth"
							target="_blank"
							rel="noopener noreferrer"
						>
					...
					{stats.githubStars > 0 && (
						<a
							href="https://github.com/Reign-Labs/reign-auth"
							target="_blank"
							rel="noopener noreferrer"
						>
							<div className="flex items-center gap-1.5 px-2.5 py-1 hover:bg-foreground/4 rounded-sm transition-colors text-foreground/50 dark:text-foreground/50">
				...
				<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
					...
					<div className="flex items-center justify-between w-full sm:w-auto sm:gap-4 shrink-0">
						<span className="text-[10px] text-foreground/50 font-mono">
							© {new Date().getFullYear()} Reign Auth Inc.
						</span>
						<div className="flex items-center gap-3 sm:gap-4">
							...
							<Link
								href="https://github.com/Reign-Labs"
								aria-label="GitHub"
								className="text-foreground/50 hover:text-foreground/80 transition-colors"
							>
				...
				<div className="p-5 lg:px-8 lg:pt-20">
					...
					>
						...
						<p className="text-sm sm:text-[15px] text-foreground/80 mb-6 sm:mb-8 leading-relaxed">
							Reign Auth is an authentication framework. It provides a
