			...
			<Button
				...
				onClick={() => {
					...
					for (let i = 0; i < elements.length; i++) {
						if (elements[i].textContent === "REIGN_AUTH_SECRET=") {
							elements[i].textContent =
								`REIGN_AUTH_SECRET=${generateRandomString(32)}`;
							setGenerated(true);
							setTimeout(() => {
								elements[i].textContent = "REIGN_AUTH_SECRET=";
								setGenerated(false);
							}, 5000);
						}
					}
		...
		<div className="w-max">
			...
			>
				<img
					...
					alt="Add Reign Auth MCP to Cursor"
					height="32"
				/>
			</Link>
